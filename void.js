...
async function initEntity() {
  ...
  if (!ctx2) return;
  let character;
  try { ({ createPhantomCharacter } = await import("./app/js/character.js?v=phantom-live-20260714-256")); character = createPhantomCharacter({ small: smallScreen, preload: ["chin", "laugh", "point", "present"] }); }
  catch { return; }
