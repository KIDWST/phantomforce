...
| **Signal** | A meaningful, evidence-backed change that may require a decision: what happened, why it matters, evidence, confidence, recommended action, can-Phantom-handle-it, approval-required. | Partially exists inside `competitor-intelligence.ts` (competitor-specific signals) and `organization-pulse.ts`. Needs to become a general cross-department object, not competitor-only. |
| **Decision** | A structured recommendation the owner approves, modifies, or dismisses in one action — distinct from an Approval, which is the risk gate *after* a decision is made. | New concept. Should be built as a packaging layer over existing Signals + the agent-run/automation engines, not a new execution path. |
