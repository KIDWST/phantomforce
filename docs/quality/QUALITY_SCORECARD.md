...
|---|---:|---|---|
| Functional correctness | 4 | Cycle 20 passed 57 disposable-database API checks, a real two-organization 28-turn browser journey, 90 authenticated model requests, and the 20/20 release-critical gate. | Auth recovery, billing, redirects, and deeper module interactions remain under-tested. |
...
| Reliability | 4 | Server uses fail-closed tenant routes; instant chat has live and forced-outage gates; relevant-thread recall, browser context, and business records survive long chat, reload, and round-trip organization switches without contamination. | Broader slow-network states and connector failures need browser-level testing. |
