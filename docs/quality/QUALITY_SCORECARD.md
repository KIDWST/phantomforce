...
|---|---:|---|---|
| Functional correctness | 3 | First real run executed 65 backend calls with 0 failures; 17 app modules mounted in UI sweep; build/typecheck pass; instant fallback, PhantomPlay, PhantomStore, and local-customer auth route tests pass. | Many flows still untested in browser, especially DB-auth recovery, billing, redirects, full interaction mobile, and permission denied. |
...
| Responsive behavior | 2 | The 2026-07-18 responsive matrix passed 46 Chrome/CDP cases across seven app destinations and six viewport widths, including Media Lab Edit and Settings Plan & access interactions on phone and desktop. | More interaction-level mobile checks, screenshot review, keyboard, dialogs, and game/editor controls still need deeper proof. |
...
| Reliability | 3 | Server uses fail-closed preview routes; build and feature tests exist; instant chat fallback and local-customer signup/login/reset/logout are covered by route-level regressions. | Broader provider health and slow/failing network states need browser-level testing. |
...
| Test coverage | 3 | Many targeted scripts exist for auth, planner, CRM, proposals, PhantomPlay, PhantomStore, local-customer auth, automations, memory, responsive viewports, etc. | Browser interaction/E2E, accessibility, and public-site tests are still thin. |
