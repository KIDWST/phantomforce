...
|---|---:|---|---|
| Functional correctness | 4 | Cycle 30 passed 57 disposable-database API checks, a real two-organization 116-turn browser journey, a 187-request local-model gate, and the 20/20 release-critical gate. | Billing, redirects, expired-session recovery, and deeper module interactions remain under-tested. |
...
| Reliability | 4 | Server uses fail-closed tenant routes; chat survives dashboard/workspace DOM replacement; relevant-thread recall, browser context, and business records survive long chat, reload, and round-trip organization switches without contamination. | Broader slow-network, expired-session, and connector failures need browser-level testing. |
...
| Test coverage | 4 | Targeted suites now include real database login, 116-turn aligned browser chat, 187-request live-model evaluation, isolated vague follow-ups, exact object/list/causal/comparison/event/membership references, scoped corrections, durable-memory reload, two-organization CRUD, tamper rejection, and desktop/mobile screenshots. | Accessibility, public-site behavior, and deeper editor/game interactions are still thin. |
