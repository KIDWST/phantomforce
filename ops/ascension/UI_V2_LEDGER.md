...
- Slice 5: color-literal sweep — `phantom-tokens.css` gained `--neon-rgb: 91, 76, 255` (comma-triplet form) alongside the existing `--neon: #5b4cff`; every `rgba(91, 76, 255, X)` / `rgba(91,76,255, X)` in phantom.css, phantomplay.css, orggraph.css, phantom-skin.css became `rgba(var(--neon-rgb), X)`, and every bare `#5b4cff` became `var(--neon)` (phantom-skin.css's own `--uv` alias token now derives from `--neon` instead of duplicating the literal). ~1,050 literal occurrences replaced, brace-balance-verified identical before/after, zero literals remain in the four swept files. **Deliberately excluded `app/index.html`**: its inline `<head><style>` block and canvas `addColorStop()` calls are self-contained boot-screen styling that paints before `phantom-tokens.css` loads (or, for canvas, never resolves CSS custom properties at all) — swapping those to `var()` would break the pre-token-load paint / silently fail the canvas gradient, so those 9 occurrences stay hardcoded on purpose. Build `phantom-live-20260721-5`. Verified: brace counts unchanged per file, no `var(var(` double-wrapping, no raw literals left in swept files, `node --check` clean on all JS.
...
2. ~~Sweep hardcoded rgba(91,76,255,...) accent literals~~ DONE — slice 5: `--neon-rgb` token added, ~1,050 literals swept in phantom.css/phantomplay.css/orggraph.css/phantom-skin.css (index.html's boot-screen block intentionally excluded — see slice note). Build phantom-live-20260721-5.
