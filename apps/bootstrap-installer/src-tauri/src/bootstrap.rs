...
/// Spawn the locally-built PhantomBot desktop binary, then close the installer
...
pub async fn launch_hermes_desktop(
    ...
    let exe_path = resolve_hermes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built PhantomBot desktop at {}. The desktop build \
             step may have been skipped or failed. Re-run PhantomBot Setup or \
             open the install log for details.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching PhantomBot desktop");
...
pub(crate) fn resolve_hermes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "PhantomBot.exe"),
            ("win-arm64-unpacked", "PhantomBot.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/PhantomBot.app/Contents/MacOS", "PhantomBot"),
            ("mac-arm64/PhantomBot.app/Contents/MacOS", "PhantomBot"),
        ]
    } else {
        &[("linux-unpacked", "PhantomBot")]
    };
...
pub(crate) fn resolve_hermes_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        // .../PhantomBot.app/Contents/MacOS/PhantomBot -> .../PhantomBot.app
...
/// path so a bare re-open just opens PhantomBot instead of re-running setup.
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hermes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built PhantomBot desktop app")
    })?;
...
async fn run_bootstrap(
    ...
    // Copy ourselves to HERMES_HOME/phantombot-setup.exe so the desktop app can
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("PhantomBot"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../PhantomBot.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("PhantomBot.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("PhantomBot");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/PhantomBot.app). A regression in
