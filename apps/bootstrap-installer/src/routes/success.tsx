...
import { launchPhantomBot } from '../store'

/*
 * Success screen. PHANTOMBOT stays as the visual anchor
 ...
 * PhantomBot.exe doesn't exist). We catch the Tauri error and surface it
 * inline rather than silently doing nothing — the previous version
 * had a fire-and-forget launch callback which swallowed
  ...
  return (
    <div className="phantombot-fade-in flex h-full flex-col items-center justify-center gap-8 px-12 py-10">
      <div className="w-full max-w-2xl min-w-0 text-center">
        <BrandMark aria-hidden="true" className="phantombot-hero-mark mx-auto mb-6 size-24" />
        <p
        ...
        >
          <span>
            <span>PhantomBot is ready</span>
          </span>
          <span aria-hidden="true">PhantomBot is ready</span>
        </p>

        <p className="m-0 text-center text-base leading-normal tracking-tight text-muted-foreground">
          Your private local operator is installed and ready to launch.
        </p>
      </div>

      <HackeryButton
        disabled={launching}
        label={launching ? 'Launching' : 'Launch PhantomBot'}
        loading={launching}
