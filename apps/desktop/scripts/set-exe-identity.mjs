...
// built Windows executable using rcedit, decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked executable keeps the stock Electron icon and
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'file-version': PACKAGE_JSON.version,
    'product-version': PACKAGE_JSON.version,
    'version-string': {
      ProductName: PRODUCT_NAME,
      FileDescription: PRODUCT_NAME,
      CompanyName: COMPANY_NAME,
      LegalCopyright: `Copyright (c) 2026 ${COMPANY_NAME}`
    }
  })

  console.log(`[set-exe-identity] done — ${PRODUCT_NAME} icon + identity stamped`)
}
