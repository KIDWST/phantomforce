...
function resolveDmgPath() {
  const fallback = `${PRODUCT_NAME}-${PACKAGE_JSON.version}-mac-${ARCH}.dmg`
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, fallback)
  }
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, fallback)
}

function resolveNsisPath() {
  // electron-builder artifactName is '${productName}-${version}-${os}-${arch}.${ext}'.
