...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'PhantomBot is ready', message: `${provider} connected.` })
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'PhantomBot could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
