...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    ...
    expect(await screen.findByText('Set up PhantomBot')).toBeTruthy()
    expect(screen.getByText('Connect to an existing PhantomBot engine')).toBeTruthy()
    expect(screen.getByText('Install the PhantomBot engine locally')).toBeTruthy()
    expect(screen.queryByText(/steps complete/i)).toBeNull()
    expect(screen.queryByText(/Fetching installer manifest/i)).toBeNull()
  })

  it('continues local bootstrap only when the local PhantomBot engine is selected', async () => {
    ...
    fireEvent.click(await screen.findByText('Install the PhantomBot engine locally'))

    expect(desktop.continueBootstrapLocal).toHaveBeenCalledTimes(1)
    expect(screen.getByText('Set up PhantomBot')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up PhantomBot')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install the PhantomBot engine locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText('Local installation could not start. Restart PhantomBot and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install the PhantomBot engine locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(screen.queryByText('Local installation could not start. Restart PhantomBot and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install the PhantomBot engine locally')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart PhantomBot and try again.')
    ).toBeTruthy()
    ...
    expect(screen.queryByText('Local installation could not start. Restart PhantomBot and try again.')).toBeNull()
  })
  ...
  it('opens the remote connection form from the first-run choice', async () => {
    ...
    fireEvent.click(await screen.findByText('Connect to an existing PhantomBot engine'))
