...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    ...
    expect(screen.getByText('Connect an existing backend')).toBeTruthy()
    expect(screen.getByText('Install the local engine')).toBeTruthy()
  ...
  it('continues local bootstrap only when Install Hermes locally is selected', async () => {
    ...
    fireEvent.click(await screen.findByText('Install the local engine'))
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install the local engine')).closest('button') as HTMLButtonElement
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install the local engine')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install the local engine')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart PhantomBot and try again.')
    ).toBeTruthy()
  ...
  it('opens the remote connection form from the first-run choice', async () => {
    ...
    fireEvent.click(await screen.findByText('Connect an existing backend'))
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    ...
    expect(await screen.findByText('PhantomBot needs a one-time setup')).toBeTruthy()
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('PhantomBot needs a one-time setup')).toBeNull()
  })
})
