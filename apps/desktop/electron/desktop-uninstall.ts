 ...
 *
 *   macOS:   …/PhantomBot.app/Contents/MacOS/PhantomBot → …/PhantomBot.app
 *   Windows: …\PhantomBot\PhantomBot.exe                → …\PhantomBot
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/PhantomBot.app/Contents/MacOS/PhantomBot → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/PhantomBot.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs PhantomBot.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]PhantomBot$/i.test(dir) || /[\\/]Hermes$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) {
      return dir
    }
