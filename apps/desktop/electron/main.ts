...
let bootProgressState = {
  ...
  message: 'Waiting to start PhantomBot engine',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — PhantomBot will start automatically when it completes…',
        12
      )
    },
  ...
  try {
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'PhantomBot update did not finish',
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating PhantomBot — this window will close and the updater will open. Don’t reopen PhantomBot yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. PhantomBot will keep running — try again, or run \`hermes update\` from a terminal.`
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating PhantomBot — this window will close. Don’t reopen PhantomBot yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
async function startHermes() {
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving PhantomBot engine', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote PhantomBot engine at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote PhantomBot engine is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.spawn', `Starting PhantomBot engine via ${backend.label}`, 84)
    ...
    await advanceBootProgress('backend.port', 'Waiting for PhantomBot engine to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for PhantomBot engine to become ready', 90)
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'PhantomBot engine is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// ---------------------------------------------------------------------------
// phantombot:// deep links (e.g. phantombot://blueprint/morning-brief?time=08:00).
// Legacy hermes:// links remain accepted for compatibility. Development uses
// phantombot-dev:// so a source build never replaces the installed handler.
...
function handleDeepLink(url) {
  ...
  // phantombot://blueprint/<key>?slot=val -> host="blueprint", path="/<key>"
...
// second-instance argv. Without the lock a second `phantombot://` launch spawns a
