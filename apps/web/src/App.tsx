  ...
  async function submitCommand(event: FormEvent) {
    ...
    if (aiProvider === "openrouter_glm") {
      ...
      try {
        ...
        addActivity(
          data?.live_provider_called ? "GLM 5.2 responded" : "GLM 5.2 setup blocked",
          data?.openrouter?.blocked_reason ?? data?.openrouter?.error_message ?? "OpenRouter lane returned through Phantom AI.",
        ...
        setMessages((current) => [
          ...current,
          {
            id: makeId("msg-assistant"),
            role: "assistant",
            content: "Phantom AI could not reach the backend OpenRouter lane. Check the server and env setup.",
          },
        ]);
        addActivity("OpenRouter lane offline", "The Phantom AI backend did not answer.", "warn");
      } finally {
        ...
        <section className="chat-card">
          ...
          <form className="command-form" onSubmit={submitCommand}>
            ...
            >
              ...
              {canManageAccess ? <option value="openrouter_glm">GLM 5.2 (OpenRouter)</option> : null}
            </select>
            <input
              value={commandText}
...
function TrainerSimulationView({ canManageAccess }: { canManageAccess: boolean }) {
  ...
  return (
    <Page
      ...
      action={<TruthBadge state="demo" label="Online - protected" />}
    >
      <section className="simulation-hero">
        <div>
