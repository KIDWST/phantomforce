...
import { classifyPhantomIntent as classifyRaw, deriveActionContract } from "./intent-router.js?v=phantom-live-20260817-159";
import { baseSiteDraft, ensureSiteDesign, applyWebsitePrompt } from "./workspaces.js?v=phantom-live-20260817-159";
import { parseInvoiceRequest, createInvoiceFromDraft, invoiceCard, fmtMoneyMinor } from "./invoices.js?v=phantom-live-20260817-159";
import { buildPromptIntegrityEnvelope } from "./prompt-integrity.js?v=phantom-live-20260817-159";
import { buildAiRuntimeRequest, waitForAiRuntimeSave } from "./ai-runtime.js?v=phantom-live-20260817-159";
...
function loadRuntimeAiSettings() {
  /* brainMode defaults to "api" (Connected): the server walks a real
     provider chain (private lane, Claude CLI, OpenRouter, local Ollama) and
...
function canAskHermes(raw, intent, settings) {
  ...
  // Decisions must always resolve against the real local approval records.
  // Sending "we have two decisions" to fast chat is how a model timeout
  // turned into the dead-end response reported by the owner.
  if (intent.primaryIntent === "approval_request") return false;
...
function operatorContextAnswer(text) {
  const model = text.match(/\b(glm\s*5(?:\.2)?|glm[-\s]?\d+(?:\.\d+)?|qwen[^\s,.!?]*|claude[^\s,.!?]*|ollama|openrouter)\b/i)?.[0]?.replace(/\s+/g, " ");
  const isStatement = !/[?]$/.test(text.trim()) && /\b(we|we're|we are|i|i'm|i am|it|it's|it is|claude|glm|qwen|hermes|phantom)\b/i.test(text);
  if (isStatement) {
    return {
      say: model
...
function routeCommand(raw, settings) {
  ...
  if (/(help|what can you|how do|what do you do|\?$)/.test(s) && s.length < 60) {
    return {
      say: "Ask in plain business language. Tell me what to build, fix, automate, check, or turn into work. Create a video request, prep media, inspect leads, draft a site, or ask a normal question.",
      cards: [],
      open: null,
    };
  }
