...
import { mountAgentConsole } from "./agentops.js?v=phantom-live-20260820-190";
import { renderAutomation } from "./brandops.js?v=phantom-live-20260820-190";
import { handleCommand, handleSmartCommand, handleInvoiceRequest } from "./command.js?v=phantom-live-20260820-190";
import { esc } from "./workspaces.js?v=phantom-live-20260820-190";
import { analyzeFile, humanSize } from "./docanalyzer.js?v=phantom-live-20260820-190";
import { openInvoicePrintable } from "./invoices.js?v=phantom-live-20260820-190";
import { getMediaRetentionDays, setMediaRetentionDays, MEDIA_RETENTION_OPTIONS, loadContentAssets, contentAssetDisplayUrl, registerContentAsset } from "./contenthub.js?v=phantom-live-20260820-190";
import { setCompanionState } from "./companion.js?v=phantom-live-20260820-190";
import { mountPhantomPresence } from "./phantom-presence.js?v=phantom-live-20260820-190";
import {
...
} from "./settings.js?v=phantom-live-20260820-190";
import {
  buildPromptIntegrityEnvelope,
  MAX_PROMPT_CHARS,
  promptSizeError,
} from "./prompt-integrity.js?v=phantom-live-20260820-190";
...
function applyQueuedOperatorPrompt() {
  ...
  const queued = queuedOperatorPrompt;
  queuedOperatorPrompt = null;
  activatePhantomAiTab("chat");
  chatBindings.input.value = queued.prompt;
  chatBindings.resize();
  if (queued.autoSubmit && !runningRequest) {
    setComposerStatus(`${queued.source} handed this work to PhantomBot. Running now…`, "live", 0);
    void chatBindings.submitPrompt(queued.prompt, []);
  } else {
    setComposerStatus(
      runningRequest
        ? `${queued.source} prepared the next operation. Send it when the current run finishes.`
        : `${queued.source} prepared this operation. Review it, then send when ready.`,
      "live",
      6200,
    );
    setTimeout(() => {
      try { chatBindings?.input?.focus({ preventScroll: true }); } catch { chatBindings?.input?.focus(); }
    }, 40);
  }
  return true;
}

export function queuePhantomAiPrompt(prompt, options = {}) {
  const cleanPrompt = String(prompt || "").replace(/\u0000/g, "").trim().slice(0, MAX_PROMPT_CHARS);
  if (!cleanPrompt) return false;
  queuedOperatorPrompt = {
    prompt: cleanPrompt,
    autoSubmit: options.autoSubmit === true,
    source: cleanText(options.source || "PhantomForce", 80),
  };
  if (rootEl?.isConnected && chatBindings?.input) applyQueuedOperatorPrompt();
  return true;
}
...
function isEngineeringPrompt(prompt) {
  const engineeringText = String(prompt || "").replace(/\b(?:code[ -]?word|codename)\b/gi, "conversation label");
  return /\b(?:code|coding|codebase|repo|repository|pull request|commit|branch|typescript|javascript|backend|frontend|api|endpoint|database|migration|unit tests?|integration tests?|e2e|continuous integration)\b/i.test(engineeringText)
    || /\b(?:build|implement|fix|debug|refactor|patch|test|document)\b.{0,80}\b(?:server|function|class|module|package|bug|test suite|codebase|repo|repository)\b/i.test(engineeringText);
}
