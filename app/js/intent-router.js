...
const APPROVAL = /^(?:please\s+)?(?:approve|decline|reject|sign off(?: on)?)\b|^(?:the\s+)?(?:approval|review) queue[.!?]*$|\b(?:my|our|this workspace(?:'s)?|this business(?:'s)?|this company(?:'s)?)\b.{0,40}\b(?:approvals?|approval queue|review queue|pending decisions?)\b|\b(?:show|open|list|check|review)\b.{0,40}\b(?:approvals?|approval queue|review queue|pending decisions?|waiting on me)\b|\b(?:we|i)\s+have\s+(?:(?:\d+|one|two|three|four|five)\s+)?(?:pending\s+)?decisions?\b|\b(?:decisions?|approvals?)\b.{0,32}\b(?:need(?:s)?\s+(?:to\s+)?(?:be\s+)?handled|need(?:s)?\s+my\s+call|are\s+waiting)\b|\b(?:waiting on me|needs my call)\b/i;
