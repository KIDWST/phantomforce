...
const PLAN = /\b(?:make|create|give|draft|build)\s+(?:me\s+)?(?:an?\s+)?(?:(?:practical|simple|detailed|quick|concise|actionable|low-cost|\d+[\s-]step|[a-z]+-step)\s+){0,3}(?:plan|roadmap|breakdown|strategy)\b|\b(?:break this down|roadmap this|plan this|help me plan)\b/i;
...
const APPROVAL = /^(?:please\s+)?(?:approve|decline|reject|sign off(?: on)?)\b|^(?:the\s+)?(?:approval|review) queue[.!?]*$|\b(?:my|our|this workspace(?:'s)?|this business(?:'s)?|this company(?:'s)?)\b.{0,40}\b(?:approvals?|approval queue|review queue|pending decisions?)\b|^(?:(?:please|can you|could you|would you)\s+)?(?:show|open|list|check|review)\b.{0,40}\b(?:approvals?|approval queue|review queue|pending decisions?|waiting on me)\b|\b(?:we|i)\s+have\s+(?:(?:\d+|one|two|three|four|five)\s+)?(?:pending\s+)?decisions?\b|\b(?:decisions?|approvals?)\b.{0,32}\b(?:need(?:s)?\s+(?:to\s+)?(?:be\s+)?handled|need(?:s)?\s+my\s+call|are\s+waiting)\b|\b(?:waiting on me|needs my call)\b/i;
...
export function classifyPhantomIntent(raw = "") {
  ...
  if (/\b(handoff|operator|private lane|claude|glm|qwen|ollama|openrouter|local model|backend)\b/i.test(text)
    || (/\b(connection|disconnected|offline|down|fallback)\b/i.test(text) && /\b(ai|model|brain|hermes|phantom|claude|glm|qwen|backend|server)\b/i.test(text))) {
