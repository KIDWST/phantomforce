...
/* one place for the "/ month" · "/ year" price suffix — the public-site
   starter sells both monthly and one-time plans, so every price renderer
   shares this. */
...
export function applyWebsitePrompt(site, promptText) {
  ...
  /* public site starters: naming a template drops in the full page, then the
     ...
     this must not early-return, since "PhantomForce" appears in almost every
     legitimate prompt about this product's own site. */
...
function renderMemory(el, rerender) {
  ...
  if (brainPanel) {
    brainPanel.addEventListener("toggle", () => {
      ...
      import("./brain.js?v=phantom-live-20260717-15")
