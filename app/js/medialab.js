...
import { currentTenantId, ctx, session as accessSession, workspaceStorageGetItem, workspaceStorageRemoveItem, workspaceStorageSetItem } from "./store.js?v=phantom-live-20260817-152";
import {
...
} from "./contenthub.js?v=phantom-live-20260817-152";
import { freshEditState, applyFilterPreset, paintEdit, heuristicAiEdit, addBokehSpot, removeBokehSpotNear, estimateSubjectPoint } from "./imagefilters.js?v=phantom-live-20260817-152";
import {
...
import { loadImageForEditing, exportCanvas, requestAiEdit, requestRemoveBackground } from "./mediabackend.js?v=phantom-live-20260817-152";
import { createMediaJob, listMediaJobs, retryMediaJob, transitionMediaJob } from "./mediageneration.js?v=phantom-live-20260817-152";
import { mountVideoEditor } from "./videocut.js?v=phantom-live-20260817-152";
import { assetsAvailable, assetBlobUrl, listAssets, recordAssetUsage, saveToAssetCloud, listLocalAssets, refreshLocalAssets, localAssetBlobUrl } from "./orgs.js?v=phantom-live-20260817-152";
...
function explainMediaFailure(reason = "", detail = "", lane = "") {
  ...
  if (/enoent|not recognized|command not found|no such file/.test(text))
    return "Connect Media Lab, then Re-check";
...
function consumeEditIntent(opts = {}) {
  ...
  if (!intent || !["image", "video"].includes(intent.type) || !intent.url) return;
  try { workspaceStorageRemoveItem(EDIT_INTENT_KEY); } catch {}
  const type = intent.type === "video" ? "video" : "image";
  const asset = {
  ...
  session.edit = { url: asset.url, type, id: asset.id };
  session.tab = "edit";
  if (type === "image") resetEdit();
  ...
  const runDoctor = async (force = false) => {
    ...
    if (prov === IMAGE_MEDIA_LANE) {
      if (!h.hasToken && !h.bridgeAuth) {
        ...
        setDoctor("warn", "ChatGPT Bridge — Connect", ["Open Settings → ChatGPT Bridge"]);
      }
      ...
      setDoctor("warn", "Media Lab — Connect", ["Offline sketches only until connected"]);
    } else if (h.studio && prov === PRIMARY_MEDIA_LANE) {
      setDoctor("warn", "Media Lab — Connect", ["Media service is up", "render lane not ready"]);
    } else {
