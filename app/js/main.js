...
import { handleCommand, handleSmartCommand, commandSuggestions } from "./command.js?v=phantom-live-20260721-5";
import { WORKSPACE_DEFS, missionWidgets, esc } from "./workspaces.js?v=phantom-live-20260721-5";
import { createPhantomCharacter } from "./character.js?v=phantom-live-20260721-5";
import { renderMediaStudio, DEFAULT_PROVIDERS } from "./medialab.js?v=phantom-live-20260721-5";
import { renderContentHub, renderAnalytics } from "./contenthub.js?v=phantom-live-20260721-5";
import { mountManagedGrowthReport } from "./managedgrowth.js?v=phantom-live-20260721-5";
import { createPhantomStage3D } from "./phantom-3d.js?v=phantom-live-20260721-5";
import { renderFlowMap, flowSummary } from "./flowmap.js?v=phantom-live-20260721-5";
import { mountPhantomWire, mountAgentConsole } from "./agentops.js?v=phantom-live-20260721-5";
import { mountPhantomAI } from "./phantomai.js?v=phantom-live-20260721-5";
import { renderAutomation, renderDeveloperAutopilotPanel, renderDeveloperAgentRunsPanel } from "./brandops.js?v=phantom-live-20260721-5";
import { renderVacationMode, cachedVacationStatus } from "./vacation.js?v=phantom-live-20260721-5";
import { renderSiteStudio } from "./sitestudio.js?v=phantom-live-20260721-5";
import { renderPromptLibrary } from "./promptlibrary.js?v=phantom-live-20260721-5";
import { mountCompanion, setCompanionState, setCompanionMode, companionMode, refreshCompanionCore } from "./companion.js?v=phantom-live-20260721-5";
import { brainLiveLabel, onBrainState } from "./brain-state.js?v=phantom-live-20260721-5";
import { mountDesktopContextWidget } from "./desktop-context.js?v=phantom-live-20260721-5";
import { renderOperatorMiniSettings, renderOperatorSettings } from "./settings.js?v=phantom-live-20260721-5";
import { getRembgStatus, getMediaEngineHealth } from "./mediabackend.js?v=phantom-live-20260721-5";
import { mountBuddy, buddyReact } from "./buddy.js?v=phantom-live-20260721-5";
import { mountAmbient } from "./ambient.js?v=phantom-live-20260721-5";
import { renderCompetitorIntelligence } from "./competitor-intelligence.js?v=phantom-live-20260721-5";
import {
...
import { renderAssetCloud } from "./assetcloud.js?v=phantom-live-20260721-5";
import { assetsAvailable } from "./orgs.js?v=phantom-live-20260721-5";
import { renderPhantomStore } from "./phantomstore.js?v=phantom-live-20260721-5";
import { renderPhantomPlay } from "./phantomplay.js?v=phantom-live-20260721-5";
...
import { renderPhantomPlay as renderPhantomPlayV2 } from "./phantomplay-v2.js?v=phantom-live-20260721-5";
const phantomPlayV2Opted = () => { try { return localStorage.getItem("pf.phantomplay.v2") === "1"; } catch { return false; } };
import { pageWorkerHtml, mountPageWorkers } from "./pageworker.js?v=phantom-live-20260721-5";
import {
  customizeNavigation,
  loadOrganizationCustomization,
} from "./customization.js?v=phantom-live-20260721-5";
import { mountMissionControl } from "./missioncontrol.js?v=phantom-live-20260721-5";
...
const BASE_NAV = [
  ...
  { id: "phantomai",  label: "Phantom AI",   icon: "chat",  ws: "phantomai" },
...
function renderConsoleCoreStatus(activeCompanionMode = companionMode()) {
  ...
  const guard = settings.externalActionMode === "blocked"
    ...
    : settings.externalActionMode === "owner_rules"
      ? "Owner rules"
      : "Approval gate";
  /* Live truth from the last real reply (brain-state.js): configured mode
     stays first, but a degraded/offline brain is never hidden behind it. */
  const live = brainLiveLabel();
  const brainDetail = live ? `${brainMode} · ${live.text}` : brainMode;
  return `
    <span><b>Brain</b><i${live && live.tone !== "ok" ? ` class="core-${esc(live.tone)}"` : ""}>${esc(brainDetail)}</i></span>
    <span><b>Hands</b><i>${esc(hands)}</i></span>
    <span><b>Guard</b><i>${esc(guard)}</i></span>`;
}
onBrainState(() => refreshCompanionCore());
...
let activeMode = "ask";
const POSE_VERSION = "phantom-live-20260721-5";
let phantom3d = null;
...
function openWorkspace(id, pushHash = true) {
  ...
  overlayRoot.innerHTML = `
    <div class="overlay ${def.wide ? "overlay-wide" : ""}" role="dialog" aria-modal="true" aria-label="${esc(def.title)}">
      ...
      <section class="overlay-panel" tabindex="-1">
        <header class="overlay-head">
          <div>
  ...
  /* Modal contract: background shell is inert, focus moves into the dialog. */
  document.querySelector("[data-phantom]")?.setAttribute("inert", "");
  overlayRoot.querySelector(".overlay-panel")?.focus({ preventScroll: true });
...
function closeOverlay(clearHash) {
  ...
  overlayRoot.innerHTML = "";
  document.body.classList.remove("overlay-open");
  document.querySelector("[data-phantom]")?.removeAttribute("inert");
...
document.addEventListener("keydown", (e) => {
  ...
  const items = [...overlayRoot.querySelectorAll(OVERLAY_FOCUSABLE)].filter((n) => n.offsetParent !== null);
  if (!items.length) return;
  const first = items[0];
  const last = items[items.length - 1];
  const inside = overlayRoot.contains(document.activeElement);
  if (e.shiftKey && (document.activeElement === first || !inside)) { e.preventDefault(); last.focus(); }
  else if (!e.shiftKey && (document.activeElement === last || !inside)) { e.preventDefault(); first.focus(); }
});
document.querySelector("[data-skip-link]")?.addEventListener("click", (e) => {
  e.preventDefault();
  const main = document.querySelector("[data-console]");
