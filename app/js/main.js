...
} from "./store.js?v=phantom-live-20260914-228";
import {
  loadOrganizationPulse, loadBrainContract, pulseAttentionItems, brainContractAttentionItems, cachedOrganizationPulse,
} from "./organizationpulse.js?v=phantom-live-20260914-228";
import { handleCommand, handleSmartCommand } from "./command.js?v=phantom-live-20260914-228";
import { WORKSPACE_DEFS, missionWidgets, esc, selectAccountingTab } from "./workspaces.js?v=phantom-live-20260914-228";
import { renderOrganizationPanel } from "./organization.js?v=phantom-live-20260914-228";
import { createPhantomCharacter } from "./character.js?v=phantom-live-20260914-228";
import { renderUnifiedAnalytics } from "./analytics-hub.js?v=phantom-live-20260914-228";
import { renderMediaStudio } from "./medialab.js?v=phantom-live-20260914-228";
import { createPhantomStage3D } from "./phantom-3d.js?v=phantom-live-20260914-228";
import { renderFlowMap, flowSummary } from "./flowmap.js?v=phantom-live-20260914-228";
import { mountPhantomWire, mountAgentConsole } from "./agentops.js?v=phantom-live-20260914-228";
import { activatePhantomAiTab, mountPhantomAI, queuePhantomAiPrompt } from "./phantomai.js?v=phantom-live-20260914-228";
import { renderPhantomHunter } from "./phantomhunter.js?v=phantom-live-20260914-228";
import { renderAutomation, renderDeveloperAutopilotPanel, renderDeveloperAgentRunsPanel, selectAutomationSection } from "./brandops.js?v=phantom-live-20260914-228";
import { renderPlanner } from "./planner.js?v=phantom-live-20260914-228";
import { renderVacationMode, cachedVacationStatus } from "./vacation.js?v=phantom-live-20260914-228";
import { renderSiteStudio } from "./sitestudio.js?v=phantom-live-20260914-228";
import { renderPromptLibrary } from "./promptlibrary.js?v=phantom-live-20260914-228";
import { setCompanionState, setCompanionMode, companionMode, refreshCompanionCore } from "./companion.js?v=phantom-live-20260914-228";
import { mountDesktopContextWidget } from "./desktop-context.js?v=phantom-live-20260914-228";
import { getOperatorInfrastructureStatus, getOperatorSettings, hydrateOperatorRuntimeSettings, renderOperatorMiniSettings, renderOperatorSettings } from "./settings.js?v=phantom-live-20260914-228";
import { getRembgStatus, getMediaEngineHealth } from "./mediabackend.js?v=phantom-live-20260914-228";
import { mountAmbient } from "./ambient.js?v=phantom-live-20260914-228";
import { renderCompetitorIntelligence } from "./competitor-intelligence.js?v=phantom-live-20260914-228";
import { registerContentAsset, renderContentHub } from "./contenthub.js?v=phantom-live-20260914-228";
import { renderChicagoShotsStudio } from "./chicagoshots-studio.js?v=phantom-live-20260914-228";
import {
...
} from "./orgs.js?v=phantom-live-20260914-228";
import { renderPhantomStore } from "./phantomstore.js?v=phantom-live-20260914-228";
import { renderPhantomPlay } from "./phantomplay.js?v=phantom-live-20260914-228";
...
import { renderPhantomPlay as renderPhantomPlayV2 } from "./phantomplay-v2.js?v=phantom-live-20260914-228";
...
} from "./customization.js?v=phantom-live-20260914-228";
import { mountMissionControl } from "./missioncontrol.js?v=phantom-live-20260914-228";
import { initCommandOS, applyCommandExecutionMode } from "./command-os.js?v=phantom-live-20260914-228";
import { createRouteRegistry } from "./product-grammar.js?v=phantom-live-20260914-228";
...
const WORKSPACE_STYLE_BUNDLES = Object.freeze({
  phantomplay: [
    "/app/phantomplay.css?v=phantom-live-20260914-228",
    "/app/phantomplay-v2.css?v=phantom-live-20260914-228",
  ],
  phantomstore: ["/app/phantomstore.css?v=phantom-live-20260914-228"],
  phantomai: [
    "/app/creator-studio.css?v=phantom-live-20260914-228",
    "/app/automation-next.css?v=phantom-live-20260914-228",
  ],
  phantomhunter: [
    "/app/phantomhunter.css?v=phantom-live-20260914-228",
    "/app/phantomhunter-connect.css?v=phantom-live-20260914-228",
  ],
  media: ["/app/creator-studio.css?v=phantom-live-20260914-228"],
  content: ["/app/creator-studio.css?v=phantom-live-20260914-228"],
  chicagoshots: ["/app/chicagoshots-studio.css?v=phantom-live-20260914-228"],
});
...
function showWorkspaceTransition(transition, phase = "loading") {
  ...
  transition.node.innerHTML = `<div class="workspace-transition-card">
    <div class="workspace-transition-mark" aria-hidden="true"><img src="/app/assets/brand-phantom.png?v=phantom-live-20260914-228" alt=""><i></i><i></i></div>
...
function ownerHasPhantomAccess() {
  return isOwnerOperator() || isAdmin() || isLiveAdminHost();
}
...
const openNavTabs = new Set(["dashboard", "media", "content", "analytics", "leads", "bookings", "money", "sites"]);
...
const WORKSPACE_ALIASES = {
  ...
  riskwatch: "automation",
  protect: "automation",
  memory: "memory",
  analytics: "analytics",
};
const ROUTE_REGISTRY = createRouteRegistry([
  { id: "dashboard", title: "Dashboard", authority: "member" },
], WORKSPACE_ALIASES);
...
function mobileNavFocusables(sidebar) {
  if (!sidebar) return [];
  return $$(MOBILE_NAV_FOCUSABLE, sidebar).filter((element) => {
...
function setMobileNav(open, { restoreFocus = false } = {}) {
  ...
  const compact = !!sidebar && window.matchMedia("(max-width: 900px)").matches;
  ...
  renderMobileBottomNav();
  const mobileDock = $("[data-mobile-bottom-nav]");
  if (compact && mobileDock) mobileDock.inert = mobileNavOpen;
  if (mobileNavOpen && !wasOpen) {
    requestAnimationFrame(() => {
      const first = mobileNavFocusables(sidebar)[0] || sidebar;
      focusWithoutScroll(first);
    });
  } else if (!mobileNavOpen && wasOpen && restoreFocus) {
    requestAnimationFrame(() => focusWithoutScroll($("[data-mobile-more]")));
  }
}
...
function goNav(id) {
  ...
  if (!item) return;
  if (!canUseNavItem(item)) {
    try { localStorage.setItem("pf.settings.tab.v1", "plan"); } catch {}
    activeNav = "settings";
    renderNav();
    renderWorkspacePage("settings", true);
    speak(`${item.label} is not part of this workspace version. Plan & access is open so you can switch tiers.`, "", "alert");
    return;
  }
...
function renderOsMenu() {
  ...
  const primaryIds = ["dashboard", "leads", "comms", "bookings", "money", "media", "content", "sites", "analytics", "phantomai", "automation", "intelligence", "runtime", "phantomhunter", "audit", "notifications", "planner", "vacation", "memory", "settings", "adminos", "developer", "phantomplay", "phantomstore"];
  const items = orderedNavItems()
...
const ACCOUNT_TIERS = [
  ...
  },
  {
    ...
    price: "$2,500/mo",
    badge: "Growth",
    copy: "Operator-grade system for growth: native creative workflow, accounting visibility, and owner controls.",
...
function renderConsoleCoreStatus(activeCompanionMode = companionMode()) {
  ...
  const providerNames = { claude: "Claude CLI", private: "Codex CLI", chatgpt: "ChatGPT Bridge", openrouter: "OpenRouter", local: "Local / Ollama" };
...
async function hydrateLivePlan(body) {
  ...
  if (ownerHasPhantomAccess()) {
    const chip = body.querySelector(".account-plan-chip");
    if (chip) chip.innerHTML = `<span>Phantom</span><b>owner</b>`;
    mount.innerHTML = `
      <article class="account-card">
        <p class="account-card-k">Hidden owner tier</p>
        <h4>Phantom · owner mode</h4>
        <p>This account is the PhantomForce owner account. It bypasses customer plan restrictions and keeps every module, developer surface, and live-ops control available.</p>
        <div class="account-facts">
          <span><b>access</b>all modules</span>
    ...
    <div class="account-plan">
      ...
      <section class="account-grid">
        ...
        <article class="account-card account-payment">
          ...
          <p>Connect payments once, then manage billing activity from this account.</p>
          <div class="account-actions">
            <button class="btn btn-primary" data-open-ws="settings" data-settings-target="media">Connect payments</button>
      ...
      <section class="account-section">
        ...
        <div class="account-billing-list">
          ...
          <span><b>Payment connection</b><i>Choose Connect payments to securely link your provider.</i></span>
          <span><b>Next action</b><i>Connect payments or request an invoice.</i></span>
        </div>
      </section>
      <section class="account-section">
        <div class="set-sec-head">
...
let activeMode = "ask";
const POSE_VERSION = "phantom-live-20260914-228";
let phantom3d = null;
...
function renderDashboardBrief() {
  ...
  const snapshot = [
    ...
    { label: "Social channels", value: String(socialChannels), detail: socialChannels ? "connected accounts" : "connect analytics", open: "analytics" },
    {
      ...
      detail: "open leads · not booked revenue",
      open: "leads",
    },
  ];
...
function accessVersionCopy() {
  if (ownerHasPhantomAccess()) {
    return { label: "Phantom", detail: "Owner mode · all modules · safe self-editing", tone: "active" };
  }
...
function opportunityCard() {
  ...
  const openLeads = visible(store.state.leads || []).filter((lead) => !["won", "lost"].includes(lead.status)).length;
  if (!growth?.available) {
    return { label: "New opportunities", value: openLeads ? `${openLeads} tracked leads` : "No signal yet", detail: openLeads ? "Prospects to qualify · not booked revenue" : "Add a prospect in Relationships to build your pipeline", open: "leads", tone: "idle" };
  }
  ...
  if (growth.proposalPipeline > 0) {
    return {
      ...
      detail: proposalAction?.title || "Open proposals · not confirmed bookings",
      open: "proposals",
      tone: "good",
    };
  }
  ...
  return { label: "New opportunities", value: openLeads ? `${openLeads} tracked leads` : "No leads yet", detail: "No open proposals · qualify prospects in Relationships", open: "leads", tone: "idle" };
}
...
/* ====================== YOUR PHANTOM WORKFORCE heartbeat ======================
   One real owner loop, backed by the tenant work graph. The UI never promotes a
   proposed action to complete on its own; it shows only the server receipt and
   its read-back verification state. */
let workforceHeartbeat = null;
let workforceHeartbeatTenant = "";
let workforceHeartbeatAt = 0;
let workforceHeartbeatInFlight = null;
let workforceHeartbeatBusy = "";

async function fetchWorkforceHeartbeat(force = false) {
  const tenant = currentTenantId();
  if (!force && workforceHeartbeat && workforceHeartbeatTenant === tenant && Date.now() - workforceHeartbeatAt < 30000) return workforceHeartbeat;
  if (workforceHeartbeatInFlight) return workforceHeartbeatInFlight;
  workforceHeartbeatInFlight = (async () => {
    const token = session.token();
    const headers = token ? { Authorization: `Bearer ${token}` } : {};
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 8000);
    try {
      const response = await fetch(`/api/workforce/heartbeat?tenant_id=${encodeURIComponent(tenant)}`, { headers, cache: "no-store", signal: controller.signal });
      const payload = await response.json().catch(() => null);
      if (!response.ok || !payload?.ok) throw new Error(payload?.error || "Workforce heartbeat unavailable.");
      workforceHeartbeat = payload.heartbeat;
      workforceHeartbeatTenant = tenant;
      workforceHeartbeatAt = Date.now();
    } catch (error) {
      if (!workforceHeartbeat || workforceHeartbeatTenant !== tenant) {
        workforceHeartbeat = { unavailable: true, error: error?.message || "Workforce heartbeat unavailable." };
        workforceHeartbeatTenant = tenant;
      }
    } finally {
      clearTimeout(timer);
      workforceHeartbeatInFlight = null;
    }
    return workforceHeartbeat;
  })();
  return workforceHeartbeatInFlight;
}

function workActionTitle(action) {
...
function renderWorkforceHeartbeat() {
  ...
  if (!host) return;
  const heartbeat = workforceHeartbeatTenant === currentTenantId() ? workforceHeartbeat : null;
  host.classList.toggle("is-unavailable", !!heartbeat?.unavailable);
  if (!heartbeat) {
    host.innerHTML = `<div class="workforce-heartbeat-loading">Reading real work, approvals, and receipts…</div>`;
    return;
  }
  if (heartbeat.unavailable) {
    const authIssue = /\b(?:authorization|bearer|unauthorized|forbidden|401|403|session)\b/i.test(String(heartbeat.error || ""));
    const headline = authIssue ? "Reconnect automation" : "Automation is checking in";
    const detail = authIssue
      ? "Your secure session needs to be refreshed before Phantom can read or run work. Nothing was sent or marked complete."
      : "Live receipts are temporarily unavailable. Phantom paused safely and did not claim any work was completed.";
    host.innerHTML = `
      <div class="workforce-heartbeat-head">
        <div><p>AUTOMATION STATUS</p><h2>${headline}</h2><span>${authIssue ? "Secure connection required" : "Safe retry in progress"}</span></div>
        <b class="workforce-heartbeat-state">${authIssue ? "SETUP NEEDED" : "PAUSED SAFELY"}</b>
      </div>
      <div class="workforce-heartbeat-recovery">
        <div><b>${authIssue ? "Reconnect once. Phantom handles the rest." : "No action was taken without proof."}</b><p>${detail}</p></div>
        <div class="workforce-heartbeat-actions">
          ${authIssue ? `<button type="button" data-open-ws="settings">Connection settings</button>` : ""}
          <button type="button" data-workforce-retry>Try again</button>
        </div>
      </div>`;
    host.querySelector("[data-workforce-retry]")?.addEventListener("click", async () => {
      workforceHeartbeat = null;
      renderWorkforceHeartbeat();
      await fetchWorkforceHeartbeat(true);
      renderWorkforceHeartbeat();
    });
  ...
  if (!needs.length && !blocked.length && !verified.length && !openTasks && focus?.kind === "clear") {
    host.innerHTML = `<div class="workforce-heartbeat-head"><div><p>YOUR PHANTOM WORKFORCE</p><h2>No decisions waiting</h2><span>Work queue is clear. Provider setup may still be needed.</span></div><div class="workforce-heartbeat-actions"><button type="button" data-open-ws="automation">Open Automations</button></div></div><details class="workforce-clear-proof"><summary>Work queue evidence</summary><p>${esc(focus.evidence || `Work graph version ${heartbeat.version || "current"}.`)}</p></details>`;
    return;
  }
  const primaryState = needs.length
    ? `${needs.length} decision${needs.length === 1 ? "" : "s"} need you`
    : blocked.length
      ? `${blocked.length} blocked item${blocked.length === 1 ? "" : "s"} explained`
      : "Nothing urgent needs you";
  host.innerHTML = `
    <div class="workforce-heartbeat-head">
      <div><p>YOUR PHANTOM WORKFORCE</p><h2>What Needs Me</h2><span>${esc(primaryState)}</span></div>
      <div class="workforce-heartbeat-actions">
        ${heartbeat.canApproveAllSafe ? `<button class="workforce-approve-all" type="button" data-workforce-approve-all ${workforceHeartbeatBusy ? "disabled" : ""}>Approve all safe <b>${Number(heartbeat.safeBulkCount || 0)}</b></button>` : ""}
        <button type="button" data-open-ws="approvals">Review in Automations</button>
      </div>
    </div>
    ${focus ? `
    ...
    <div class="workforce-heartbeat-flow" aria-label="Work lifecycle">
      ...
      <span class="${verified.length ? "is-good" : ""}"><i>03</i><b>Verified complete</b><small>${verified.length || "none yet"}</small></span>
      <span class="${openTasks ? "is-good" : ""}"><i>04</i><b>Nothing slips</b><small>${openTasks ? `${openTasks} tracked` : "clear"}</small></span>
    </div>
    <div class="workforce-heartbeat-body">
      <div class="workforce-needs-list">
        ${needs.length ? needs.slice(0, 3).map((action) => `
          <article data-work-action-id="${esc(action.id)}">
            <div><span>${esc(String(action.type || "work").replaceAll(".", " "))}</span><b>${esc(workActionTitle(action))}</b><p>${esc(action.rationale || "Ready for owner review.")}</p></div>
            <div class="workforce-decision-actions">
              <button class="is-approve" type="button" data-work-action-decision="approve" ${workforceHeartbeatBusy ? "disabled" : ""}>Approve &amp; run</button>
              <button type="button" data-work-action-decision="reject" ${workforceHeartbeatBusy ? "disabled" : ""}>Reject</button>
            </div>
          </article>`).join("") : `
          <article class="workforce-clear-state"><span>MONITORING</span><b>Phantom is watching the business.</b><p>No verified approval request is waiting right now.</p></article>`}
      </div>
      <div class="workforce-receipt-list">
        ${blocked.slice(0, 2).map((action) => `
          <article class="is-blocked"><span>BLOCKED — EXACT REASON</span><b>${esc(workActionTitle(action))}</b><p>${esc(action.receipt?.blockedReason || action.receipt?.summary || "Execution could not continue.")}</p><small>${esc(action.receipt?.remediation || "Review the connection and retry.")}</small></article>`).join("")}
        ${verified.slice(0, Math.max(1, 3 - Math.min(2, blocked.length))).map((action) => `
          <article class="is-verified"><span>VERIFIED COMPLETE</span><b>${esc(workActionTitle(action))}</b><p>${esc(action.receipt?.summary || "Write verified.")}</p><small>receipt ${esc(action.receipt?.id || "recorded")} · ${esc(action.receipt?.verifiedAt ? ago(action.receipt.verifiedAt) : "verified")}</small></article>`).join("")}
        ${!blocked.length && !verified.length ? `<article class="workforce-receipt-empty"><span>RECEIPTS</span><b>Execution evidence appears here.</b><p>Phantom reports complete only after the resulting record reads back successfully.</p></article>` : ""}
      </div>
    </div>`;
  ...
  host.querySelector("[data-workforce-approve-all]")?.addEventListener("click", async () => {
    ...
    workforceHeartbeatBusy = "all";
    renderWorkforceHeartbeat();
    try {
      const token = session.token();
      const response = await fetch("/api/workforce/actions/decide-all-safe", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
        body: JSON.stringify({ tenant_id: currentTenantId() }),
      });
      const payload = await response.json().catch(() => null);
      if (!response.ok || !payload?.ok) throw new Error(payload?.error || "Safe bulk decision failed.");
      speak(`${payload.completed?.length || 0} safe action${payload.completed?.length === 1 ? "" : "s"} executed and verified. ${payload.skipped_external ? `${payload.skipped_external} external action${payload.skipped_external === 1 ? " remains" : "s remain"} gated.` : "No external action was included."}`, "", "bright");
      await fetchWorkforceHeartbeat(true);
    } catch (error) {
      speak(`Bulk decision failed: ${error?.message || "service unreachable"}. Unverified actions remain open.`, "", "alert");
    } finally {
      workforceHeartbeatBusy = "";
      renderWorkforceHeartbeat();
    }
...
/* Decision Cards are the owner-state layer over the server's cross-department
   ...
   Modify create a verified internal follow-up task and may then navigate to
   the recommended surface. Nothing external executes from this deck. */
...
function renderDecisions() {
  ...
  deck.innerHTML = `
    <div class="decision-head">
      ...
      <button class="decision-review-all" type="button" data-decision-all ${decisionBusyId ? "disabled" : ""}>Approve all ${open.length}</button>
      <button class="decision-review-all" type="button" data-open-ws="approvals">Open Automation Decisions</button>
    </div>
...
async function decideAllDecisions() {
  if (decisionBusyId || !(decisionFeed?.open || []).length) return;
  decisionBusyId = "all";
  renderDecisions();
  try {
    const token = session.token();
    const response = await fetch("/phantom-ai/decisions/decide-all", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
      body: JSON.stringify({ tenant_id: currentTenantId() }),
    });
    const payload = await response.json().catch(() => null);
    if (!response.ok || !payload?.ok) throw new Error(payload?.error || "Decision queue could not be completed.");
    await Promise.all([fetchDecisions(true), fetchWorkforceHeartbeat(true)]);
    speak(`${payload.verified_internal_actions || 0} recommendation${payload.verified_internal_actions === 1 ? "" : "s"} turned into verified follow-up work. No external action was sent.`, "", "bright");
    renderWorkforceHeartbeat();
  } catch (error) {
    speak(`The decision queue did not complete: ${error?.message || "server unreachable"}. Unverified items remain visible.`, "", "alert");
  } finally {
    decisionBusyId = "";
    renderDecisions();
  }
...
function dashWidgetItems() {
  ...
  return [
    { id: "automation", ws: "automation", label: "Automation Control", icon: "auto",
      val: pendingApprovals || "", alert: pendingApprovals > 0,
      sub: pendingApprovals ? `${pendingApprovals} decision${pendingApprovals === 1 ? "" : "s"} waiting` : "Workflows clear" },
...
function renderDashWidgets() {
  ...
  if (!host) return;
  /* Respect the same access and entitlement rules the sidebar uses: a widget
     for a plan-disabled feature is omitted instead of shown as a dead card. */
  const items = dashWidgetItems().filter((item) => {
    const navItem = NAV.find((n) => n.id === item.id);
    return navItem ? canUseNavItem(navItem) && !navFeatureDisabled(navItem) : true;
  });
  host.innerHTML = items.map((item) => `
...
const QUICK = [
  ...
  { label: "Open PhantomBot", icon: "spark",   open: "phantomai" },
...
function renderConsole() {
  ...
  renderWorkforceHeartbeat();
  fetchWorkforceHeartbeat().then(() => renderWorkforceHeartbeat()).catch(() => {});
  renderDecisions();
...
function wireDeck() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (e.target.closest("[data-mobile-more]")) {
      const willOpen = !mobileNavOpen;
      setMobileNav(willOpen, { restoreFocus: !willOpen });
      return;
    }
    ...
    if (e.target.closest("[data-decision-all]")) { decideAllDecisions(); return; }
    ...
    if (opener) {
      ...
      if (opener.dataset.settingsFocus) {
        try { sessionStorage.setItem("pf.settings.connection.focus.v1", opener.dataset.settingsFocus); } catch {}
      }
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (phantom.hidden) return;
    if (e.key === "Tab" && mobileNavOpen && window.matchMedia("(max-width: 900px)").matches) {
      const sidebar = $(".sidebar");
      const focusable = mobileNavFocusables(sidebar);
      if (sidebar && focusable.length) {
        const active = document.activeElement;
        const first = focusable[0];
        const last = focusable[focusable.length - 1];
        if (!sidebar.contains(active) || (!e.shiftKey && active === last) || (e.shiftKey && active === first)) {
          e.preventDefault();
          focusWithoutScroll(e.shiftKey ? last : first);
        }
      }
      return;
    }
    ...
    else if (e.key === "Escape" && mobileNavOpen) { e.preventDefault(); setMobileNav(false, { restoreFocus: true }); }
...
const mediaOpts = () => ({
  ...
  runAI: (prompt, options = {}) => {
    const queued = queuePhantomAiPrompt(prompt, { ...options, source: options.source || "PhantomForce Autopilot" });
    if (queued) routeWorkspace("phantomai");
    return queued;
  },
...
function providerManagerMarkup(manager) {
  ...
  const labels = { ["co" + "dex_cli"]: "Codex CLI", claude_cli: "Claude CLI", chatgpt_bridge: "ChatGPT Bridge", openrouter_glm: "OpenRouter", local_ollama: "Local / Ollama" };
  ...
  body.innerHTML = `
    <div class="developer-shell dev-shell">
      ...
      <section class="developer-card">
        ...
        <p class="set-note">Legacy diagnostic lane aliases. The organization-wide AI Brain choice in Settings is authoritative for live prompts.</p>
        <div class="developer-lane-groups">
          <div class="developer-lane-group developer-lane-group-wide">
            ...
            <p class="developer-lane-saved" data-dev-lane-saved hidden>Legacy alias saved locally. Organization AI Brain remains authoritative.</p>
          </div>
...
function phantomBotBrainPresentation() {
  const settings = getOperatorSettings();
  const route = settings.phantomBot;
  const status = getOperatorInfrastructureStatus("phantombot");
  const providerNames = { deepseek: "DeepSeek", local: "Phantom V1 / Ollama", private: "Codex CLI", claude: "Claude CLI", openrouter: "OpenRouter", chatgpt: "ChatGPT Bridge" };
  const explicitProvider = providerNames[route.provider] || route.provider || "AI brain";
  const explicitModel = String(route.models?.[route.provider] || "model not selected");
  const provider = route.providerMode === "smart"
    ? "Phantom Hybrid"
    : route.providerMode === "multiple"
      ? "Phantom Blend"
      : explicitProvider;
  const model = route.providerMode === "smart"
    ? "Automatic routing"
    : route.providerMode === "multiple"
      ? `${route.selectedProviders?.length || 0} providers`
      : explicitModel;
  return {
    provider,
    model,
    status: status.label || "AI brain · Checking",
...
const CUSTOM = {
  ...
  automation: { title: "Automations", kicker: "PhantomBot control plane — workflows, decisions, and exceptions", custom: true, wide: true, render: (body) => renderAutomation(body, mediaOpts()) },
  ...
  phantomhunter: { title: "PhantomHunter", kicker: "One connected repository. Provider-confirmed active keys only.", custom: true, wide: true, render: (body) => renderPhantomHunter(body) },
  ...
  phantomai: {
    ...
    wide: true,
    render: (body) => {
      const brain = phantomBotBrainPresentation();
      body.innerHTML = `
        <div class="phantomai phantombot-os" data-phantombot-os>
          ...
          <section class="phantombot-stage">
            ...
            <section class="phantombot-constellation" data-phantombot-constellation aria-label="Mission constellation">
              <header>
                <div><span>MISSION CONSTELLATION</span><b data-phantombot-constellation-objective>Describe the outcome you want PhantomBot to own.</b></div>
                <small data-phantombot-constellation-progress>0 / 5 confirmed</small>
              </header>
              <ol data-phantombot-constellation-nodes></ol>
              <footer>
                <button type="button" data-phantombot-open-mission><span>✦</span><b>Inspect mission</b></button>
                <button type="button" data-phantombot-mission-next hidden><span>↗</span><b>Next move</b></button>
                <button type="button" data-phantombot-copy-continuity><span>⧉</span><b>Copy continuity</b></button>
                <button type="button" data-phantombot-brain-mesh><i aria-hidden="true"></i><span><b>Mapping mesh</b><small>Checking real connections</small></span></button>
              </footer>
            </section>
              ...
              <div class="phantomai-chat" data-phantomai-chat-mount>
                ...
                <section class="phantombot-session-menu" data-phantombot-session-menu hidden role="dialog" aria-modal="true" aria-labelledby="phantombot-session-control-title" tabindex="-1">
                  ...
                  <div>
                    ...
                    <button type="button" data-phantombot-copy-continuity>Copy continuity</button>
                    <button type="button" data-phantombot-branch-active>Parallel path</button>
                    <button type="button" data-phantombot-archive-session>Archive</button>
                  </div>
                  <p>Continuity packets carry the objective, grounded context, proof, and safest next move without inventing completion.</p>
                </section>
              </div>
            </div>

            <div class="phantomai-pane phantombot-panel-pane" data-phantomai-pane="automations" hidden>
              <header class="phantombot-panel-head">
                <div><span>Internal operations</span><h2>Automations</h2></div>
                <button type="button" data-phantomai-tab="chat">Back to chat</button>
              </header>
              <div class="phantombot-automations-mount" data-phantombot-automations-mount></div>
            </div>
            ...
            <aside class="phantombot-context-drawer" data-phantombot-context-drawer hidden role="dialog" aria-modal="true" aria-labelledby="phantombot-session-details-title" tabindex="-1">
              ...
              <nav role="tablist" aria-label="Session details">
                <button type="button" role="tab" data-phantombot-detail-tab="mission">Mission</button>
...
let openId = null;
Object.entries({ ...WORKSPACE_DEFS, ...CUSTOM }).forEach(([id, surface]) => {
  ROUTE_REGISTRY.register({
    id,
    title: surface.title,
    authority: surface.ownerOnly ? "owner" : surface.adminOnly ? "admin" : "member",
    entitlement: surface.entitlement || null,
    surface,
  });
});

function workspaceDef(id) {
  return ROUTE_REGISTRY.resolve(id)?.surface || null;
}
...
async function renderWorkspacePage(id, pushHash = true) {
  ...
  if (activeWorkspaceTransition !== transition || transition.version !== workspaceTransitionVersion) return false;
  const wsMood = key === "automation" ? { mood: "talking", emotion: "bright" } : { mood: "listening", emotion: "bright" };
...
function routeWorkspace(id, pushHash = true) {
  ...
  if (key === "automation") {
    selectAutomationSection(requestedKey);
  ...
  if (!ROUTE_REGISTRY.has(key)) {
    renderDashboardPage(false);
    if (location.hash) {
      try { history.replaceState(null, "", "#page/dashboard"); } catch {}
    }
    speak("That workspace is unavailable. Dashboard is open instead.", "", "alert");
    return false;
  }
...
async function openWorkspace(id, pushHash = true) {
  ...
  if (activeWorkspaceTransition !== transition || transition.version !== workspaceTransitionVersion) return false;
  const overlayMood = key === "automation" ? { mood: "talking", emotion: "bright" } : { mood: "listening", emotion: "bright" };
...
async function resolveBootSession() {
  const restored = isLiveAdminHost() ? await verifyLiveSession() : resolveSession();
  if (!restored) return null;
  if (isLocalDevHost() && new URLSearchParams(location.search).has("session")) return restored;
  const token = session.token();
  if (token) {
    try {
  ...
  try {
    ...
    if (response.ok && (auth.databaseAuthEnabled || auth.ownerProductionAuthEnabled || auth.gatewayForwardedAuthEnabled)) {
      session.clear();
      return null;
    }
  } catch {
    // Preserve the local visual shell when no backend is reachable at all.
  }
  return restored;
}

async function boot() {
  ctx.session = await resolveBootSession();
  wireDeck();
  initCommandOS();
  store.onChange(() => {
    if (!phantom.hidden) {
