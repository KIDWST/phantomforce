...
} from "./imagefilters.js?v=phantom-live-20260712-227";
import { getRembgStatus, requestRemoveBackground, probeAiEditBackend, requestAiEdit, loadImageForEditing, loadImage, exportCanvas, syncAssetUpload, listSyncedAssets, fetchSyncedAssetFile } from "./mediabackend.js?v=phantom-live-20260712-227";
import { addCustomDailyIdea, dailyIdeaState, refreshDailyIdeas, saveIdeaForLater } from "./content-ideas.js?v=phantom-live-20260712-227";
import { parseAnalyticsReport } from "./social-analytics.js?v=phantom-live-20260712-227";
import {
...
} from "./content-editor.js?v=phantom-live-20260712-227";
import {
...
function activeWorkspaceSites() {
  try {
    return visible(store.state.sites || []).filter((site) => !site.archived);
  } catch {
    return [];
  }
}
function recentWorkspaceActivity() {
  try {
    return visible(store.state.activity || []).filter((row) =>
      /\b(creator|content|media|website|publish|site|analytics)\b/i.test(`${row.who || ""} ${row.text || ""}`));
  } catch {
    return [];
  }
}
