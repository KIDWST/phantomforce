...
} from "./imagefilters.js?v=phantom-live-20260914-219";
import { archiveSyncedAsset, getRembgStatus, requestRemoveBackground, probeAiEditBackend, requestAiEdit, loadImageForEditing, loadImage, exportCanvas, syncAssetUpload, listSyncedAssets, fetchSyncedAssetFile, restoreSyncedAsset } from "./mediabackend.js?v=phantom-live-20260914-219";
import { addCustomDailyIdea, dailyIdeaState, refreshDailyIdeas, saveIdeaForLater } from "./content-ideas.js?v=phantom-live-20260914-219";
import { approveAndSubmitContentPublication, fetchContentPublishingStatus, listContentPublications, persistContentPublication } from "./contentpublication.js?v=phantom-live-20260914-219";
import { parseAnalyticsReport } from "./social-analytics.js?v=phantom-live-20260914-219";
import {
...
} from "./content-editor.js?v=phantom-live-20260914-219";
import {
...
import { socialConnectorsFromResponse, socialPreflightFromResponse } from "./social-connection-state.js?v=phantom-live-20260914-219";
...
const CH_MEDIA_RETENTION_KEY = "pf.medialab.retentionDays.v1";
const DAY = 864e5;
...
export function getMediaRetentionDays() {
  let saved = 30;
  try {
    const raw = workspaceStorageGetItem(CH_MEDIA_RETENTION_KEY);
    saved = raw === null ? 30 : Number(raw);
  } catch {}
  return MEDIA_RETENTION_OPTIONS.some((option) => option.days === saved) ? saved : 30;
}
export function setMediaRetentionDays(days) {
  const next = Number(days);
  const value = MEDIA_RETENTION_OPTIONS.some((option) => option.days === next) ? next : 30;
  try { workspaceStorageSetItem(CH_MEDIA_RETENTION_KEY, String(value)); } catch {}
  return value;
}
export const CONTENT_ASSET_LIMITS = Object.freeze({
  get retentionDays() { return getMediaRetentionDays(); },
  maxItems: 2000,
  budgetBytes: 8000000,
...
function normalizeContentAsset(input = {}) {
  ...
  return {
    ...
    createdAt,
    expiresAt: CONTENT_ASSET_LIMITS.retentionDays ? createdAt + CONTENT_ASSET_LIMITS.retentionDays * DAY : 0,
    url,
    trimmed,
    live: !!input.live,
    saved: !!input.saved,
...
function pruneContentAssets(items = []) {
  const cutoff = CONTENT_ASSET_LIMITS.retentionDays ? Date.now() - CONTENT_ASSET_LIMITS.retentionDays * DAY : 0;
  const seen = new Set();
  const ordered = items
...
function normalizeRecycledContentAsset(input = {}) {
  ...
  return {
    ...asset,
    trashedAt,
    trashExpiresAt: CONTENT_ASSET_LIMITS.retentionDays ? trashedAt + CONTENT_ASSET_LIMITS.retentionDays * DAY : 0,
  };
}
function pruneRecycledContentAssets(items = []) {
  ...
  return items
    ...
    .filter((asset) => !CONTENT_ASSET_LIMITS.retentionDays || asset.trashExpiresAt > now)
...
export function restoreRecycledContentAssets(ids = []) {
  ...
  saveContentAssets([
    ...restored.map(({ trashedAt, trashExpiresAt, ...asset }) => ({
      ...
      expiresAt: CONTENT_ASSET_LIMITS.retentionDays ? restoredAt + CONTENT_ASSET_LIMITS.retentionDays * DAY : 0,
...
function renderContentPlanner(body, data, esc, root, opts) {
  ...
  body.innerHTML = `
    ...
    <section class="ch-card ch-planner-connectors">
      <div class="ch-card-h"><div><h3>Connect your workday</h3><span class="ch-src">Email and calendar connections</span></div></div>
...
function wirePostPublish(body, data, assets, esc, root, opts) {
  ...
  const saveDraft = async (status) => {
    ...
    notify(status === "scheduled"
      ...
      : status === "posted"
        ? `${publication ? "Manual post record saved to this business" : "Manual post record preserved locally"} and added to Analytics. Connect the publishing account in Settings to post live.${syncError ? ` (${syncError})` : ""}`
...
async function refreshAnalyticsOAuthSetup({ force = false } = {}) {
  ...
  try {
    ...
    analyticsOAuthSetupState.error = error?.message || "Social account setup could not be checked.";
  } finally {
...
async function saveAnalyticsOAuthAppSetup({ platform, clientId, clientSecret, redirectUri }) {
  ...
  analyticsOAuthSetupState.setup = response?.setup || null;
  analyticsOAuthSetupState.loaded = true;
  const connectors = socialConnectorsFromResponse(response);
  analyticsConnectorState.connectors = connectors.length ? connectors : analyticsConnectorState.connectors;
...
function analyticsReadinessCopy({ hasLiveMetrics, configuredCount, oauthReadyCount, totalCount }) {
  ...
  if (configuredCount) {
    return {
      ...
      body: "Your accounts are connected. Pull the latest platform metrics now.",
      action: `<button class="btn btn-primary" type="button" data-an-sync-all>${analyticsConnectorState.loading ? "Syncing…" : "Sync live feed"}</button>`,
    };
  }
  if (oauthReadyCount) {
    return {
      tone: "ready",
      title: "Accounts are ready to connect.",
    ...
    <div class="an">
      ...
      <div class="ch-kpis an-kpis">
        ${hasLiveMetrics
          ...
          : `${kpi("Live channels", `0/${displayAccounts.length}`, "platform reporting")}${kpi("Ready", K(oauthReadyCount), "connections available")}${kpi("Connected", K(configuredCount), "accounts reporting")}${kpi("Next step", "Connect", "choose a platform below")}`}
      </div>
      <section class="an-hero">
        <div>
          ...
          <p>${hasLiveMetrics ? "This page shows live platform analytics from connected social accounts. Local media files, drafts, and uploads are not counted as social performance." : "Connect YouTube, Instagram, Facebook, TikTok, X, LinkedIn, or Pinterest once. PhantomForce syncs real metrics and keeps publishing approval-gated."}</p>
        </div>
      </section>
      <section class="an-toolbar" aria-label="Live analytics actions">
        <div class="an-hero-actions">
          ...
          <span class="an-src">${svgIc("up")} ${liveApiRows.length}/${displayAccounts.length} live · ${configuredCount}/${displayAccounts.length} connected · ${oauthReadyCount}/${displayAccounts.length} ready</span>
        </div>
      </section>
      ...
      <div class="an-section-head" data-an-sources><div><p class="ch-eyebrow">Sources</p><h3>Social account connections</h3></div><span>Reach and follower sync</span></div>
