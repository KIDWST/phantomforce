 ...
 * meaningful.
 *
 * PhantomPlay host bridge (see starter-template.html): each DAY is one
 * "run" — 'complete' fires once at day-end with that day's coin total
 * as score, guarded per-day since "Next Day" starts a fresh run in the
 * same iframe session. Best-run persistence rides in `state` the same
 * way as PhantomCube (sandboxed localStorage throws — verified).
 */
(function () {
  "use strict";
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "restore" && e.data.state && typeof e.data.state.bestCoins === "number") { best = e.data.state; showBest(); }
  });
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) { clearInterval(patienceTimer); ovenRunning = false; cancelAnimationFrame(ovenRaf); host("paused", { paused: true }); }
  });
  ...
  function loadBest() {
    if (embedded) return { bestCoins: 0, bestDay: 0 }; // filled in by `restore` if the host has prior state
    try { return JSON.parse(localStorage.getItem(SAVE_KEY)) || { bestCoins: 0, bestDay: 0 }; } catch { return { bestCoins: 0, bestDay: 0 }; }
  }
  function saveBest(b) {
    if (embedded) return; // persisted via the state field on the next score/progress/complete message instead
    try { localStorage.setItem(SAVE_KEY, JSON.stringify(b)); } catch {}
  }
  let best = loadBest();
  ...
  function servePizza(bakePos) {
    ...
    phase = "between";
    renderHud();
    host("score", { score: coins, progress: Math.round((served / ORDERS_PER_DAY) * 100), state: { day, served, coins } });
    setTimeout(() => {
  ...
  let dayCompleteSent = false; // host requires 'complete' exactly once per run — "Next Day" starts a fresh run
  function endDay() {
    phase = "day-end";
    ...
    if (!dayCompleteSent) { dayCompleteSent = true; host("complete", { score: total, progress: 100, state: { day, coins, best } }); }
    dayOverlay.hidden = false;
  }

  function startDay(n) {
    day = n; served = 0; dayReport = []; dayCompleteSent = false;
    ...
    newOrder();
    host("progress", { progress: 0, state: { day, served: 0, coins } });
  }
