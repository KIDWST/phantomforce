...
(function () {
  ...
  function servePizza(bakePos) {
    ...
    phase = "between";
    renderHud();
    host("score", { score: coins, progress: Math.round((served / ORDERS_PER_DAY) * 100), state: { day, served, lastVerdict: r.bakeVerdict } });
    setTimeout(() => {
  ...
  function endDay() {
    ...
    host("progress", { score: coins, progress: 100, state: { day, coins, dayComplete: true } });
  }
