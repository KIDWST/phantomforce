 ...
 * level-select grid, beat-one-to-unlock-the-next progression persisted
 * in localStorage.
 */
(function () {
  "use strict";
  ...
  function onWin() {
    ...
    const starCount = Object.keys(progress.done).length;
    const pct = Math.round((starCount / LEVELS.length) * 100);
    if (starCount >= LEVELS.length) host("complete", { score: starCount * 100, progress: 100, state: { levelsCleared: starCount } });
    else host("score", { score: starCount * 100, progress: pct, state: { levelsCleared: starCount, level: levelIndex } });
  }
  ...
  window.__PhantomCubeTest = {
    ...
    resetProgress() { progress = { unlocked: 1, done: {} }; saveProgress(progress); },
  };
})();
