 ...
 *
 * Perf discipline (matching phantom-ages/game.js): fixed-timestep
 * simulation decoupled from render rate, pooled particles/projectiles,
 * a cached Path2D for the track so it isn't rebuilt every frame, and
 * no per-frame allocation in the hot loop.
 */
(function () {
  "use strict";

  // PhantomPlay host bridge (see starter-template.html for the
  // contract): 1P races kart[0]'s finishing place into a real score
  // (podium points, not invented); 2P scores the better of the two
  // human finishes. Each race is one "run" — 'complete' fires once at
  // the phase→"finished" transition, which by construction happens
  // exactly once per race (simulateTick stops advancing once the
  // phase leaves "racing"). No cross-session progress to persist —
  // each race starts fresh, same as reloading any local kart racer.
  const embedded = window.parent !== window;
  const host = (type, data = {}) => { if (embedded) parent.postMessage({ source: "phantomplay-game", type, ...data }, "*"); };
  let hostSound = true, hostReducedMotion = false, hostPaused = false;
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "pause") hostPaused = true;
    if (e.data.type === "resume") hostPaused = false;
  });
  document.addEventListener("visibilitychange", () => { hostPaused = document.hidden; if (document.hidden) host("paused", { paused: true }); });
  const PLACE_SCORE = { 1: 100, 2: 75, 3: 50, 4: 25 };

  // Inlined rather than a shared cross-directory script — PhantomPlay
  // games run sandboxed and self-contained, one directory per game.
  class ObjectPool {
    ...
    get pooledCount() { return this.free.length; }
  }
  ...
  function startRace(mode) {
    ...
    state.raceT = 0;
    host("progress", { progress: 0, state: { mode } });
  }
  ...
  function simulateTick(dt) {
    ...
    if (Math.floor(state.raceT) !== Math.floor(state.raceT - dt)) {
      const leadHuman = state.karts.find((k) => k.human);
      if (leadHuman) host("progress", { progress: Math.min(95, Math.round((leadHuman.unwrapped / (LAPS * TRACK.N)) * 100)), state: { raceT: Math.round(state.raceT) } });
    }
    ...
    if (done) {
      ...
      const humanRanks = standings.filter((k) => k.human).map((k) => k.rank);
      const bestRank = humanRanks.length ? Math.min(...humanRanks) : 4; // no human kart is unreachable in real play; treat defensively as last
      host("complete", { score: PLACE_SCORE[bestRank] || 10, progress: 100, state: { mode: state.mode, ranks: humanRanks } });
    }
  }
  ...
  function frame(now) {
    ...
    if (!hostPaused && (state.phase === "racing" || state.phase === "countdown")) {
      accumulator += delta;
