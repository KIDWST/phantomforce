"""V11R8 first-frame composition repair for Phantom Ages and PhantomStrike.
...
PATCH_TAG = "PhantomProductionWorldV11R8"
GROUND_MESH = "/Game/Phantom/Generated/Common/V11R8/SM_V11R8_GroundPatch"
SAVED = os.path.abspath(unreal.Paths.project_saved_dir())
REPORT = os.path.join(SAVED, "PhantomProductionWorldsV11R8.json")
...
def ensure_ground_mesh():
    ...
    if not unreal.EditorAssetLibrary.duplicate_asset("/Engine/BasicShapes/Plane", GROUND_MESH):
        raise RuntimeError("Could not create the V11R8 project ground mesh")
    mesh = unreal.EditorAssetLibrary.load_asset(GROUND_MESH)
    if not mesh:
        raise RuntimeError("V11R8 ground mesh did not load")
...
def spawn_surface(label, mesh, material, location, scale):
    ...
    if not actor:
        raise RuntimeError(f"Could not spawn V11R8 surface: {label}")
...
def spawn_mesh(label, mesh_path, location, scale, yaw=0.0, ground_z=4.0, collision=False):
    ...
    if not mesh:
        raise RuntimeError(f"Missing V11R8 authored asset: {mesh_path}")
    actor = actors.spawn_actor_from_class(
        unreal.StaticMeshActor,
    ...
    if not actor:
        raise RuntimeError(f"Could not spawn V11R8 actor: {label}")
    component = actor.get_editor_property("static_mesh_component")
    component.set_static_mesh(mesh)
...
def patch_ages(ground_mesh):
    ...
    if not grass or not dirt or not rock_material:
        raise RuntimeError("Missing V11R8 Ages production material")
    ...
    for index, y in enumerate(range(-12000, 12001, 4000)):
        label = f"AGES_V11R8_Grass_{index:02d}"
    ...
    for index, y in enumerate((-4700.0, -700.0, 3300.0)):
        ...
        label = f"AGES_V11R8_BattleBand_{index:02d}"
    ...
    for index, x in enumerate(range(-15000, 15001, 2500)):
        label = f"AGES_V11R8_BrokenRoad_{index:02d}"
    ...
    for index in range(38):
        ...
        label = f"AGES_V11R8_Foreground_{index:02d}"
...
def patch_strike(ground_mesh):
    ...
    if not concrete:
        raise RuntimeError("Missing V11R8 Strike concrete material")
    ...
    for index, y in enumerate((-2850.0, 2850.0)):
        label = f"STRIKE_V11R8_Shoulder_{index:02d}"
    ...
    for index, (x, y) in enumerate(positions):
        ...
        label = f"STRIKE_V11R8_Cover_{index:02d}"
    ...
    for index, y in enumerate((-980.0, 980.0)):
        label = f"STRIKE_V11R8_Roadblock_{index:02d}"
    ...
    for index, x in enumerate((-7800.0, -6600.0, -5400.0, -4200.0, -3000.0)):
        for side in (-1.0, 1.0):
            label = f"STRIKE_V11R8_Light_{index:02d}_{'L' if side < 0 else 'R'}"
...
results = {"revision": "V11R8", "status": "RUNNING"}
try:
    ...
    unreal.log_error("PHANTOM V11R8 WORLD PATCH FAILED: " + str(exc))
    raise
finally:
    with open(REPORT, "w", encoding="utf-8") as handle:
        json.dump(results, handle, indent=2)

unreal.log("PHANTOM V11R8 WORLD PATCH PASS")
