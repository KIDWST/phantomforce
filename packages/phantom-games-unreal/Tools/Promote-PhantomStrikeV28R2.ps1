...
$CandidateRoot = Assert-ExactPath `
    (Join-Path $ProjectRoot 'CandidateBuilds\V28R2\phantom-strike') `
    (Join-Path $ProjectRoot 'CandidateBuilds\V28R2\phantom-strike') `
    'PhantomStrike candidate root'
$candidateMarker = Join-Path $CandidateRoot 'PHANTOM_V28R2_CANDIDATE.txt'
$manifestPath = Join-Path $CandidateRoot 'PHANTOM_V28R2_MANIFEST.json'
...
if ($markerText -notmatch 'game=phantom-strike' -or $markerText -notmatch 'visual_profile=blackridge-natural-first-person-v28r2') {
    throw 'V28 candidate identity marker is invalid.'
}
$manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
if ([string]$manifest.revision -cne $Revision -or [string]$manifest.game -cne 'phantom-strike' -or [string]$manifest.visual_profile -cne 'blackridge-natural-first-person-v28r2') {
...
$backupPath = Assert-PathWithin (Join-Path $BackupRoot "phantomplay-phantom-strike-$safePreviousRevision-to-v28r2-$timestamp") $BackupRoot 'Rollback checkpoint'
...
try {
    ...
    $installedMarker = Join-Path $InstalledRoot 'PHANTOM_V28R2_INSTALLED.txt'
    Move-Item -LiteralPath (Join-Path $InstalledRoot 'PHANTOM_V28R2_CANDIDATE.txt') -Destination $installedMarker
    $promotedUtc = [DateTime]::UtcNow.ToString('o')
    Set-Content -LiteralPath $installedMarker -Encoding UTF8 -Value @(
        'PHANTOM V28R2 INSTALLED'
        'game=phantom-strike'
        "promoted_utc=$promotedUtc"
        'visual_profile=blackridge-natural-first-person-v28r2'
