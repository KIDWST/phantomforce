...
$CandidateRoot = Assert-ExactPath `
    (Join-Path $ProjectRoot 'CandidateBuilds\V27R1\phantom-strike') `
    (Join-Path $ProjectRoot 'CandidateBuilds\V27R1\phantom-strike') `
    'PhantomStrike candidate root'
$candidateMarker = Join-Path $CandidateRoot 'PHANTOM_V27R1_CANDIDATE.txt'
$manifestPath = Join-Path $CandidateRoot 'PHANTOM_V27R1_MANIFEST.json'
if (-not (Test-Path -LiteralPath $candidateMarker -PathType Leaf)) { throw 'V27 candidate marker is missing.' }
if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) { throw 'V27 candidate manifest is missing.' }
$markerText = Get-Content -LiteralPath $candidateMarker -Raw
if ($markerText -notmatch 'game=phantom-strike' -or $markerText -notmatch 'visual_profile=blackridge-photoreal-target-v27') {
    throw 'V27 candidate identity marker is invalid.'
}
$manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
if ([string]$manifest.revision -cne $Revision -or [string]$manifest.game -cne 'phantom-strike' -or [string]$manifest.visual_profile -cne 'blackridge-photoreal-target-v27') {
    throw 'V27 candidate manifest identity is invalid.'
}
...
if ($candidateSummary.file_count -ne ([int]$manifest.file_count_before_manifest + 1)) {
    throw "V27 candidate file-count mismatch: $($candidateSummary.file_count)."
}
...
$backupPath = Assert-PathWithin (Join-Path $BackupRoot "phantomplay-phantom-strike-$safePreviousRevision-to-v27r1-$timestamp") $BackupRoot 'Rollback checkpoint'
...
try {
    ...
    $installedMarker = Join-Path $InstalledRoot 'PHANTOM_V27R1_INSTALLED.txt'
    Move-Item -LiteralPath (Join-Path $InstalledRoot 'PHANTOM_V27R1_CANDIDATE.txt') -Destination $installedMarker
    $promotedUtc = [DateTime]::UtcNow.ToString('o')
    Set-Content -LiteralPath $installedMarker -Encoding UTF8 -Value @(
        'PHANTOM V27R1 INSTALLED'
        'game=phantom-strike'
        "promoted_utc=$promotedUtc"
        'visual_profile=blackridge-photoreal-target-v27'
