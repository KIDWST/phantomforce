...
$Artifact = [IO.Path]::GetFullPath((Join-Path $ProjectRoot "BuildArtifacts\$Revision\cubetown"))
$Candidate = [IO.Path]::GetFullPath((Join-Path $ProjectRoot "CandidateBuilds\$Revision\cubetown"))
...
foreach ($TargetPath in @($Artifact, $Candidate)) {
    if (-not $TargetPath.StartsWith($ProjectRoot, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Unsafe Cubetown candidate target: $TargetPath"
    }
...
Write-Host "CUBETOWN_${Revision}_PACKAGE_PASS $Candidate" -ForegroundColor Green
