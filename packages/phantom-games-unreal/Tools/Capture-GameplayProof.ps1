...
foreach($g in $games){
    ...
    $shippingExe=Get-ChildItem (Join-Path $dir 'PhantomGames\Binaries\Win64') -File -Filter '*-Win64-Shipping.exe' -ErrorAction SilentlyContinue |
        Select-Object -First 1
    if(-not $shippingExe){throw "Gameplay proof Shipping executable missing under: $dir"}
    ...
    if($shot){
        $captureSuffix=if($g.id -eq 'cubetown' -and $CubetownCaptureMode -eq 'lair'){'-LAIR-GAMEPLAY.png'}else{'-GAMEPLAY.png'}
