"""V11R10 first-frame composition repair for Phantom Ages and PhantomStrike.
...
PATCH_TAG = "PhantomProductionWorldV11R10"
REPLACED_PATCH_TAGS = {
    "PhantomProductionWorldV11R8",
    "PhantomProductionWorldV11R9",
    PATCH_TAG,
}
GROUND_MESH = "/Game/Phantom/Generated/Common/V11R10/SM_V11R10_GroundPatch"
SAVED = os.path.abspath(unreal.Paths.project_saved_dir())
REPORT = os.path.join(SAVED, "PhantomProductionWorldsV11R10.json")
...
def ensure_ground_mesh():
    ...
    if not unreal.EditorAssetLibrary.duplicate_asset("/Engine/BasicShapes/Plane", GROUND_MESH):
        raise RuntimeError("Could not create the V11R10 project ground mesh")
    mesh = unreal.EditorAssetLibrary.load_asset(GROUND_MESH)
    if not mesh:
        raise RuntimeError("V11R10 ground mesh did not load")
...
def spawn_surface(label, mesh, material, location, scale):
    ...
    if not actor:
        raise RuntimeError(f"Could not spawn V11R10 surface: {label}")
...
def spawn_mesh(label, mesh_path, location, scale, yaw=0.0, ground_z=4.0, collision=False):
    ...
    if not mesh:
        raise RuntimeError(f"Missing V11R10 authored asset: {mesh_path}")
    actor = actors.spawn_actor_from_class(
        unreal.StaticMeshActor,
    ...
    if not actor:
        raise RuntimeError(f"Could not spawn V11R10 actor: {label}")
    component = actor.get_editor_property("static_mesh_component")
    component.set_static_mesh(mesh)
...
def patch_ages(ground_mesh):
    ...
    if not grass or not dirt or not rock_material:
        raise RuntimeError("Missing V11R10 Ages production material")
    ...
    for index, y in enumerate(range(-12000, 12001, 4000)):
        label = f"AGES_V11R10_Grass_{index:02d}"
    ...
    for index, y in enumerate((-4700.0, -700.0, 3300.0)):
        ...
        label = f"AGES_V11R10_BattleBand_{index:02d}"
    ...
    for index, x in enumerate(range(-15000, 15001, 2500)):
        label = f"AGES_V11R10_BrokenRoad_{index:02d}"
    ...
    for index in range(38):
        ...
        label = f"AGES_V11R10_Foreground_{index:02d}"
...
def patch_strike(ground_mesh):
    ...
    if not concrete or not asphalt:
        raise RuntimeError("Missing V11R10 Strike production material")
    ...
    for actor in actors.get_all_level_actors():
        ...
        if component:
            ...
            restored_roads.append(label)

    # Two textured wear tracks preserve the authored road mesh and center markings while adding
    # close-range asphalt detail under the player's first thirty meters of movement.
    for index, x in enumerate((-8400.0, -7200.0, -6000.0, -4800.0, -3600.0, -2400.0)):
        for side in (-1.0, 1.0):
            label = f"STRIKE_V11R10_Wear_{index:02d}_{'L' if side < 0 else 'R'}"
            spawn_surface(label, ground_mesh, asphalt, (x, side * 360.0, 156.0), (13.5, 2.2, 1.0))
            added.append(label)

    # Staggered road-repair plates give the insertion lane close-range surface
    # detail and route rhythm without adding collision or obstructing combat.
    # Their short spans preserve the authored dashed center line.
    for row, x in enumerate((-8650.0, -8250.0, -7850.0, -7450.0, -7050.0, -6650.0)):
        for column, y in enumerate((-780.0, 0.0, 780.0)):
            if (row + column) % 3 == 1:
                continue
            label = f"STRIKE_V11R10_Repair_{row:02d}_{column:02d}"
            spawn_surface(label, ground_mesh, concrete, (x, y, 158.0), (2.6, 1.55, 1.0))
            added.append(label)
    ...
    for index, y in enumerate((-2850.0, 2850.0)):
        label = f"STRIKE_V11R10_Shoulder_{index:02d}"
    ...
    for index, (x, y) in enumerate(positions):
        ...
        label = f"STRIKE_V11R10_Cover_{index:02d}"
    ...
    for index, y in enumerate((-980.0, 980.0)):
        label = f"STRIKE_V11R10_Roadblock_{index:02d}"
    ...
    for index, x in enumerate((-7800.0, -6600.0, -5400.0, -4200.0, -3000.0)):
        for side in (-1.0, 1.0):
            label = f"STRIKE_V11R10_Light_{index:02d}_{'L' if side < 0 else 'R'}"
    ...
    for index, (x, y) in enumerate(((-7300.0, -1200.0), (-7300.0, 1200.0), (-6700.0, -1450.0), (-6700.0, 1450.0))):
        label = f"STRIKE_V11R10_NearCover_{index:02d}"
        spawn_mesh(
            label,
    ...
    return {
        ...
        "restored_roads": restored_roads,
        "added": added,
    }


results = {"revision": "V11R10", "status": "RUNNING"}
try:
    ...
    unreal.log_error("PHANTOM V11R10 WORLD PATCH FAILED: " + str(exc))
    raise
finally:
    with open(REPORT, "w", encoding="utf-8") as handle:
        json.dump(results, handle, indent=2)

unreal.log("PHANTOM V11R10 WORLD PATCH PASS")
