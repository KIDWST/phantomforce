...
for (const [label, source, contract] of [
  ...
  ["silent WebView installation", bundleConfig, "webview_install_mode = { OfflineInstaller = { silent = true } }"],
  ["existing machine runtime detection", windowsInstaller, "ReadRegStr $R0 HKLM"],
  ["existing user runtime detection", windowsInstaller, "ReadRegStr $R0 HKCU"],
  ["existing runtime skips setup", windowsInstaller, "Goto WebViewReady"],
