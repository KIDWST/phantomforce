...
const gameSlugs = ["neon-drift", "signal-match", "focus-stack", "word-weld", "reflex-grid", "rift-frenzy", "serpent-surge", "pixel-bloom", "type-storm", "im-baked", "cipher-keep"];
...
const phantomRumbleCover = read("../app/assets/phantomplay/phantom-rumble-cover.svg");
...
const persistDevSandboxBlock = module.match(/async function persistDevSandboxOverride[\s\S]*?(?=\nasync function saveDevSandboxOverride)/u)?.[0] || "";
...
assert.match(flagshipCatalog, /id:\s*"cubetown"[\s\S]*version:\s*"5\.8\.0-unreal"/u, "CubeTown catalog metadata must advertise the canonical Unreal build.");
assert.match(module, /id:\s*"phantom-ages"[\s\S]*native\/phantom-ages\?engine=unreal&version=5\.8[\s\S]*version:\s*"5\.8\.0-unreal"[\s\S]*progressSupport:\s*true/u, "Frontend catalog must launch the canonical Unreal Phantom Ages build.");
assert.match(flagshipCatalog, /id:\s*"phantom-ages"[\s\S]*unrealPlayerUrl\("phantom-ages"\)[\s\S]*version:\s*"5\.8\.0-unreal"[\s\S]*progressSupport:\s*true/u, "Server catalog must match the canonical Unreal Phantom Ages build.");
assert.match(phantomAges, /FixedBattlefieldCameraLocation[\s\S]*ProjectionMode\s*=\s*ECameraProjectionMode::Orthographic/u, "Phantom Ages must keep its fixed orthographic strategy view.");
assert.match(phantomAges, /EPhantomAgesUnitType::Clubman[\s\S]*EPhantomAgesUnitType::SpearHunter[\s\S]*EPhantomAgesUnitType::Catapult/u, "Phantom Ages must retain distinct formation roles.");
assert.match(phantomAges, /Springald[\s\S]*Catapult/u, "Phantom Ages must retain siege and counter-siege systems.");
assert.match(phantomAges, /SetBattleSpeed[\s\S]*FMath::Clamp[\s\S]*SetGlobalTimeDilation/u, "Phantom Ages must expose bounded game-speed controls.");
...
assert.match(colorRush, /\.board::before[\s\S]*repeating-linear-gradient/u, "Color Rush board must render an active lane-stage texture instead of a blank rectangle.");
assert.match(colorRush, /\.col::after[\s\S]*color-mix\(in srgb,var\(--target\)/u, "Color Rush lanes must show target-color ambient rails before tiles spawn.");
assert.match(colorRush, /\.catch::before[\s\S]*TARGET ZONE/u, "Color Rush must label the active catch zone instead of relying on a faint dashed line.");
assert.match(colorRush, /max-height:min\(68vh,680px\)/u, "Color Rush desktop stage must cap height so it does not dominate the whole play surface.");
assert.match(colorRush, /@media\(max-width:420px\)[\s\S]*\.board\{max-height:min\(62vh,570px\)/u, "Color Rush phone stage must fit without requiring the user to zoom or scroll around the game.");
assert.match(colorRush, /function surge\(color=COLORS\[target\]\?\.c\|\|'#41ffa1'\)\{board\.style\.setProperty\('--target',color\)/u, "Color Rush arena styling must follow the current target color.");
...
assert.match(phantomRumble, /MAX_CHARGE=\.98[\s\S]*function releaseCharge\(f\)[\s\S]*attack\(f,true,heldCharge\)/u, "Phantom Rumble must preserve the actual held charge through release instead of erasing it before combat resolution.");
assert.match(phantomRumble, /wallbreaker:full[\s\S]*function rumbleBounce\(f,surface[\s\S]*PHANTOM PINBALL![\s\S]*WALL BOUNCE ×\$\{r\.bounces\}/u, "Phantom Rumble max-charge hits must arm a multi-surface wall-ricochet chain with explicit collision feedback.");
assert.match(phantomRumble, /function spawnRumbleBurst[\s\S]*function drawParticle[\s\S]*feather[\s\S]*debris/u, "Phantom Rumble must render layered sparks, feathers, and debris for major impacts.");
assert.match(phantomRumble, /createDynamicsCompressor[\s\S]*function noiseBurst[\s\S]*kind==='wall'[\s\S]*function rumbleMusicTick/u, "Phantom Rumble must retain layered compressed combat sound and an adaptive procedural brawl score.");
assert.match(phantomRumble, /OVERDRIVE![\s\S]*hype:0,overdrive:0[\s\S]*data-hype/u, "Phantom Rumble must expose a visible, gameplay-backed OVERDRIVE system.");
assert.match(phantomRumble, /__PhantomRumbleTest[\s\S]*version:'3\.0\.0-overdrive'/u, "Phantom Rumble must expose the deterministic 3.0 wallbreaker regression seam.");
assert.match(phantomRumble, /<script nonce="phantomplay-native-shell">/u, "Phantom Rumble must boot inside the installed PhantomPlay security wrapper.");
assert.match(phantomRumbleCover, /OVERDRIVE 3\.0[\s\S]*MAX RUMBLE[\s\S]*WALLBREAKER[\s\S]*CHARGE · COLLIDE · RICOCHET · OVERDRIVE/u, "Phantom Rumble must keep its custom chicken-ninja Overdrive storefront art.");
...
assert.match(vespergateIndex, /engine\.js\?v=3\.1\.0[\s\S]*game\.js\?v=3\.1\.0/u, "Vespergate must load the entire 3.1.0 module graph.");
...
assert.match(vespergateGame, /Three split cloak tails[\s\S]*Asymmetric mantle[\s\S]*Pointed hood and narrow mask[\s\S]*profession-readable silhouette/u, "Vespergate 3.1 must retain the restored illustrated bearer and readable villagers.");
assert.match(vespergateGame, /function roomObjectiveText[\s\S]*function setCheckpoint[\s\S]*function respawnAtCheckpoint/u, "Vespergate 3.1 must retain the restored objective and checkpoint systems.");
assert.match(vespergateRooms, /bellSequence[\s\S]*mirrorRelays[\s\S]*sanctum/u, "Vespergate 3.1 must retain the richer room progression systems.");
assert.match(vespergateGame, /visualProfile:\s*"living-dread-restored-v1"[\s\S]*characterProfile:\s*"pointed-hood-asymmetric-mantle-v1"/u, "Vespergate must expose its restored visual profile to native playtests.");
...
for (const [label, record] of [["frontend", frontendVespergate], ["server", serverVespergate]]) {
  assert.match(record, /launchUrl:\s*"\/app\/games\/vespergate\/index\.html\?v=3\.1\.0"/u, `${label} catalog must launch Vespergate 3.1.0.`);
  assert.match(record, /version:\s*"3\.1\.0"/u, `${label} catalog must advertise Vespergate 3.1.0.`);
  assert.match(record, /Vespershield/u, `${label} catalog must describe the restored shield system.`);
  assert.match(record, /checkpoints/u, `${label} catalog must describe the restored checkpoint system.`);
  assert.doesNotMatch(record, /Space fires/u, `${label} catalog must not advertise the retired beam binding.`);
}
...
assert.match(module, /function devProjectProblem[\s\S]*2_000_000/u, "Project saves must validate missing referenced files and enforce the server's safe size limit before launch or save.");
assert.match(module, /function fetchEditableGameSource[\s\S]*response\.ok[\s\S]*invalid HTML document/u, "A server error page must never be accepted as editable game source.");
assert.match(module, /function cleanEditableGameSource[\s\S]*internal server error[\s\S]*source\.slice\(htmlStart\)/u, "Previously poisoned local drafts must automatically remove a raw server error before their real HTML document.");
assert.match(module, /function markPlayerReady[\s\S]*player-loading[\s\S]*setAttribute\("hidden"/u, "Uploaded games without the optional PhantomPlay ready message must still leave the loading screen after the iframe loads.");
assert.match(module, /playerFrame\?\.addEventListener\("load"[\s\S]*markPlayerReady/u, "The real player iframe load event must release protocol-free uploaded games.");
assert.match(module, /function retryDevProjectSync[\s\S]*\[1200, 4000, 12000\]/u, "A locally safe project must retry transient workspace-sync failures automatically.");
assert.doesNotMatch(module, /Backend sync did not complete|Internal Server Error/u, "Raw backend failures must not replace or dominate the editor experience.");
assert.doesNotMatch(persistDevSandboxBlock, /\brender\(\)/u, "Saving from the live code drawer must not rebuild or reload the running game iframe.");
assert.match(serverIndex, /dev-mode\/:gameId\/override", \{ bodyLimit: 6 \* 1024 \* 1024 \}/u, "Dev Mode saves must accept complete multi-file game projects beyond Fastify's default request limit.");
