...
const socialSettingsSource = readFileSync(new URL("../app/js/social-settings.js", import.meta.url), "utf8");
...
assert.match(socialSettingsSource, /Editable handle or profile URL/, "Media settings must let Jordan change handles per channel.");
assert.match(socialSettingsSource, /cross-posting remain locked until OAuth\/API authorization/, "Profile handles must not imply cross-posting is authorized.");
...
assert.match(socialSettingsSource, /Developer provider setup/, "Social settings should expose the owner-only provider app setup panel.");
assert.match(socialSettingsSource, /data-oauth-setup-form/, "Media settings must include the owner-only OAuth app setup form.");
assert.match(socialSettingsSource, /\/phantom-ai\/ops\/social-oauth\/setup/, "Media settings must save provider OAuth app credentials server-side.");
assert.match(socialSettingsSource, /Callback URL for provider consoles/, "Media settings must show the exact callback URL to paste into provider consoles.");
...
assert.match(serverSource, /setupRequired/, "OAuth start conflicts must tell the UI whether provider app setup is required.");
assert.match(serverSource, /oauthPreflight/, "OAuth start conflicts must include the preflight ladder for the setup wizard.");
