...
assert.doesNotMatch(contentHubSource, /parseAnalyticsReport/);
assert.doesNotMatch(contentHubSource, /PhantomForce local site\/content signals/);
assert.doesNotMatch(contentHubSource, /Import official report/);
assert.doesNotMatch(contentHubSource, /Backup only · CSV · TSV · JSON/);
assert.match(contentHubSource, /Your analytics command center\./);
...
console.log("Social analytics live dashboard checks passed.");
