...
assert.match(contentHubSource, /workspaceChartRows/);
assert.match(contentHubSource, /workspaceBreakdownBars/);
...
assert.match(contentHubSource, /Workspace analytics are live\. Platform APIs are optional/, "Analytics must show first-party workspace data before asking for social API setup.");
assert.match(contentHubSource, /Optional live social APIs/, "Social APIs must be framed as optional enrichment, not the whole analytics page.");
assert.match(contentHubSource, /analyticsConnectorState\.error = force \? message : ""/, "Silent social API checks must not interrupt local workspace analytics.");
