...
assert.match(css, /Final compact nav guard:[\s\S]*?@media \(max-width: 900px\)[\s\S]*?\.topbar2,[\s\S]*?\.os-command-rail,[\s\S]*?\.mobile-admin-homebar\s*\{[\s\S]*?display:\s*none\s*!important/u, "Final compact guard must hide every top nav bar on phones and narrow tablets.");
...
assert.match(commandOsCss, /Final loaded mobile nav kill switch:[\s\S]*?@media \(max-width: 900px\)[\s\S]*?\.phantom \.topbar2,[\s\S]*?\.phantom \.os-command-rail,[\s\S]*?\.phantom \.mobile-admin-homebar\s*\{[\s\S]*?display:\s*none\s*!important/u, "Last-loaded Command OS CSS must hide every duplicate compact top nav surface.");
