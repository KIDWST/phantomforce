...
assert.match(main, /renderMediaStudio\(target, opts\)/u, "Media Lab must mount the complete creator studio instead of a provider-status placeholder.");
...
assert.match(index, /data-nav-id="automation"[^>]*>Automations</u, "Automations must remain directly reachable from the global command rail.");
...
assert.match(contentHub, /\["ideas", "Ideas"\][\s\S]*\["drafts", "Drafts"\][\s\S]*\["publish", "Publish"\][\s\S]*\["calendar", "Planner"\]/u, "Content Hub must retain Ideas, Drafts, Publish, and Planner.");
assert.match(phantomAi, /data-phantombot-edit-media/u, "Generated PhantomBot media must expose Edit in PhantomCut.");
assert.match(phantomAi, /pf\.contenthub\.openAsset\.v1/u, "PhantomBot media must retain the handoff into Content Hub publishing.");
assert.match(creatorStudioCss, /\.ml-choose-card/u, "Restored PhantomCut controls must retain their creator-studio presentation.");
...
assert.match(main, /\{ id: "automation",\s+label: "Automations"/u, "Automations must remain a top-level app while also being available inside PhantomBot.");
...
assert.match(commandOsCss, /\.app-main > \.console\.console-workspace\s*\{[\s\S]*?grid-row:\s*1\s*\/\s*-1\s*!important/u, "Desktop workspaces must span the full app-main grid when the status strip is hidden.");
