...
globalThis.fetch = async (url, init) => {
  ...
  if (/\/phantom-ai\/respond$/.test(target)) {
    return {
      ok: true,
      json: async () => ({
        ok: true,
        answer: `Direct answer ${capturedBodies.length}: ${body.task}`,
        effort: body.effort,
      }),
    };
  }
  assert.match(target, /\/phantom-ai\/chat$/, "casual prompts should use the chat backend");
...
for (const body of capturedBodies) {
  ...
  assert.deepEqual(body.allowed_providers, ["deepseek_api", "local_ollama", "codex_cli", "claude_cli", "openrouter_glm", "chatgpt_bridge", "hermes"]);
...
assert.deepEqual(capturedBodies.at(-1).allowed_providers, ["deepseek_api", "local_ollama", "codex_cli", "claude_cli", "openrouter_glm", "chatgpt_bridge", "hermes"]);
