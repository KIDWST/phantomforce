...
export const PHANTOMPLAY_FLAGSHIP_GAMES: PhantomPlayGame[] = [
  ...
  },
  {
    ...
    summary: "Bloons-style tower defense with free off-road Sentinel placement, one winding route, starter sentries, and Century Watch bosses.",
    description: "A lane-based tower-defense game rebuilt closer to classic Bloons-style play: one big winding route instead of split lanes, free off-road Sentinel placement, starter sentries, and a first round with one Driftling before air pressure begins. Spend Glint to place and upgrade Sentinels that auto-fire on raiders walking animated energy routes toward your Spire, then trigger Overcharge for timed damage/fire-rate buffs. Century Watch escalates toward round 100, boss mechanics rotate every 10 rounds, and Room Duel support stays network-silent through PhantomPlay rooms.",
    category: "Strategy",
    ...
    developer: "Tak",
    kind: "built_in",
    launchUrl: "/app/games/skyguard-arena/index.html?v=1.3.2",
    thumbnail: "/app/assets/phantomplay/skyguard-arena-cover.svg?v=1.0.0",
    featured: true,
    version: "1.3.2",
    controls: "Click/tap a Sentinel card, then build anywhere off the road. Q triggers Overcharge Pulse, P pauses, Escape deselects. Mouse and touch both work through the same pointer input.",
    progressSupport: true,
    scoreSupport: true,
  ...
  },
  {
    id: "crown-circuit",
    title: "Crown Circuit",
    summary: "Solo bot training plus 8-card/4-hand elixir lane duels with Obsidian Relay, Oracle slows, Ram sieges, and room support.",
    description: "A royale-style lane battler: start Solo Training against Crown Bot to learn lane pressure, elixir timing, card cycling, and tower trades across multiple arenas including Obsidian Relay, then play the same drag-and-drop troop duel in a private PhantomPlay room. Oracle cards apply real slows, Ram cards pressure towers, and Crown Bot cycles the upgraded deck.",
    category: "Strategy",
    ...
    developer: "Tak",
    kind: "built_in",
    launchUrl: "/app/games/crown-circuit.html?v=1.3.1",
    thumbnail: "/app/assets/phantomplay/crown-circuit-cover.svg?v=1.0.0",
    featured: true,
    version: "1.3.1",
    controls: "Build an eight-card deck, drag from the four-card hand onto your side, and cycle back to the next card. Solo Training starts immediately against Crown Bot; Room mode waits for player two.",
    progressSupport: true,
    scoreSupport: true,
    engine: { tier: "arena-large-map", minVersion: PHANTOMPLAY_ENGINE.version },
  },
